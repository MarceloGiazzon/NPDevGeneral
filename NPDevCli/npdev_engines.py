        ...
        "engines": [
            {
                ...
                # DISTINCT from needsServer, and the difference matters: H2Server IS a server engine,
                # but its environment is a Java process, not a container. Only the three that make
                # NPDev run `docker run` are containerized -- which is what decides whether Docker is
                # genuinely required for the engine a user just picked.
                "containerized": ENGINES[key]["containerized"],
