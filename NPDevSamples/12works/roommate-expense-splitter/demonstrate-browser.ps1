param(
    [string]$BaseUrl = "http://localhost:8112",
    [string]$ScrapForAIRoot = "",
    [int]$ScraperPort = 3022,
    [string]$OutputPath = ""
)

Set-StrictMode -Version Latest
$ErrorActionPreference = "Stop"

# Browser-level demonstration for 12works/roommate-expense-splitter: drives the
# generated vanilla-JS business UI in a real (headless) browser via ScrapForAI and
# asserts on the structured evidence -- console errors, page errors, network
# failures, unexpected external requests, screenshots. Exercises Household create,
# Member create (referencing Household via its widget: "select" reference field,
# now honored directly), then Expense create referencing both Household and the
# payer Member via their own select-widget reference fields on the same form, plus
# the splitWithRefs N:M bond via the checkbox-list multiselect widget.

. (Join-Path $PSScriptRoot "..\..\scripts\sample-common.ps1")
. (Join-Path $PSScriptRoot "..\..\scripts\browser\scrapforai-harness.ps1")

function Normalize-BaseUrl([string]$Value) { return $Value.TrimEnd("/") }

$BaseUrl = Normalize-BaseUrl $BaseUrl

try {
    $health = Invoke-RestMethod -Method Get -Uri ($BaseUrl + "/actuator/health") -TimeoutSec 5
    Info ("App reachable, health: " + $health.status)
} catch {
    throw "The generated app is not reachable at $BaseUrl. Generate + start it first (see header comment). Details: $($_.Exception.Message)"
}

# None of Household/Member/Expense declare a server-side search field, so the UI's
# filter box can't isolate this run's rows from earlier manual/automated test data --
# wipe all three tables first (Expense before Member before Household, since Expense
# references both and Member references Household) so the routine's assertions are
# genuinely true on every rerun.
$existingExpenses = @(Invoke-RestMethod -Method Get -Uri ($BaseUrl + "/api/expenses") -Headers @{ "X-API-Key" = "dev-key" } | ForEach-Object { $_ })
foreach ($x in $existingExpenses) {
    Invoke-RestMethod -Method Delete -Uri ($BaseUrl + "/api/expenses/" + $x.id) -Headers @{ "X-API-Key" = "dev-key" } | Out-Null
}
if ($existingExpenses.Count -gt 0) { Info ("Cleaned up " + $existingExpenses.Count + " pre-existing Expense row(s) before running.") }

$existingMembers = @(Invoke-RestMethod -Method Get -Uri ($BaseUrl + "/api/members") -Headers @{ "X-API-Key" = "dev-key" } | ForEach-Object { $_ })
foreach ($m in $existingMembers) {
    Invoke-RestMethod -Method Delete -Uri ($BaseUrl + "/api/members/" + $m.id) -Headers @{ "X-API-Key" = "dev-key" } | Out-Null
}
if ($existingMembers.Count -gt 0) { Info ("Cleaned up " + $existingMembers.Count + " pre-existing Member row(s) before running.") }

$existingHouseholds = @(Invoke-RestMethod -Method Get -Uri ($BaseUrl + "/api/households") -Headers @{ "X-API-Key" = "dev-key" } | ForEach-Object { $_ })
foreach ($h in $existingHouseholds) {
    Invoke-RestMethod -Method Delete -Uri ($BaseUrl + "/api/households/" + $h.id) -Headers @{ "X-API-Key" = "dev-key" } | Out-Null
}
if ($existingHouseholds.Count -gt 0) { Info ("Cleaned up " + $existingHouseholds.Count + " pre-existing Household row(s) before running.") }

$samplesRoot = Normalize-AbsolutePath (Join-Path $PSScriptRoot "..\..")
$sample = Resolve-NPDevSample -SamplesRoot $samplesRoot -SampleId "12works\roommate-expense-splitter"
$evidenceDir = Join-Path $sample.RunOutputRoot "browser"
New-Item -ItemType Directory -Force -Path $evidenceDir | Out-Null

$routineDir = Join-Path $PSScriptRoot "browser-routines"
$routines = @(Get-ChildItem -LiteralPath $routineDir -Filter "*.json" | Sort-Object Name)
if ($routines.Count -eq 0) { Fail "No browser routines found in $routineDir" }

$runStamp = (Get-Date).ToString("yyyyMMdd-HHmmss")
$sharedVars = @{
    householdName = "UITEST-HOUSEHOLD-$runStamp"
    payerName     = "UITEST-PAYER-$runStamp"
}

Initialize-ScrapForAI -Root $ScrapForAIRoot | Out-Null
$ctx = Start-ScrapForAI -AppBaseUrl $BaseUrl -Root $ScrapForAIRoot -Port $ScraperPort `
    -ArtifactDir (Join-Path "D:\WorkSpace\NPDev\Build\scrapforai-artifacts" "12works-roommate-expense-splitter")

$results = @()
try {
    foreach ($routine in $routines) {
        $name = [System.IO.Path]::GetFileNameWithoutExtension($routine.Name)
        Info ("=== Routine: " + $name + " ===")
        $result = Invoke-ScrapRoutine -Context $ctx -RoutinePath $routine.FullName -Variables $sharedVars
        Assert-RoutineGreen -Result $result -Label $name | Out-Null
        Save-RoutineEvidence -Result $result -OutDir $evidenceDir -Name $name | Out-Null
        $results += [ordered]@{
            routine     = $name
            status      = (Get-Prop $result "status")
            steps       = @($result.steps).Count
            consoleErrs = @($result.evidence.consoleErrors).Count
            pageErrs    = @($result.evidence.pageErrors).Count
            netFails    = @($result.evidence.networkFailures).Count
        }
    }
} finally {
    Stop-ScrapForAI $ctx
}

if ([string]::IsNullOrWhiteSpace($OutputPath)) {
    $OutputPath = Join-Path $evidenceDir ("browser-demo-" + $runStamp + ".json")
}
$summary = [ordered]@{
    baseUrl       = $BaseUrl
    generatedAt   = (Get-Date).ToString("o")
    scraperBase   = $ctx.BaseUrl
    artifactDir   = $ctx.ArtifactDir
    sharedVars    = $sharedVars
    routineCount  = $routines.Count
    routines      = $results
    allGreen      = $true
}
$summary | ConvertTo-Json -Depth 12 | Set-Content -LiteralPath $OutputPath -Encoding UTF8

Write-Host ""
Ok ("Evidence written to " + $OutputPath)
Ok ("Screenshots/traces under " + $ctx.ArtifactDir)
Ok ("Browser verification green across " + $routines.Count + " routine(s) (Household create; Member create via the select-widget Household reference; Expense create referencing both Household and the payer Member via their own select-widget reference fields; splitWithRefs N:M bond exercised via the multiselect checkbox widget).")
