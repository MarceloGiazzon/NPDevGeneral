...
# Browser-level demonstration for 12works/pantry-expiry-tracker: drives the generated
...
# unexpected external requests, screenshots. Exercises Item create with a nested
# object field (location), an enum, and a date field; the create Flow emits a
# custom event consumed by an orchestration rule that sends a notification.
#
# Prereq: the generated app must already be running. Generate + start it first:
#   NPDevSamples/scripts/generate-sample-app.ps1 -SampleId "12works\pantry-expiry-tracker"
#   NPDevSamples/scripts/run-sample-app.ps1 -SampleId "12works\pantry-expiry-tracker"
...
# Item declares no server-side search field, so the UI's filter box can't isolate
# this run's rows from earlier manual/automated test data -- wipe the table first
# so the routine's assertions are genuinely true on every rerun.
...
$sample = Resolve-NPDevSample -SamplesRoot $samplesRoot -SampleId "12works\pantry-expiry-tracker"
...
$ctx = Start-ScrapForAI -AppBaseUrl $BaseUrl -Root $ScrapForAIRoot -Port $ScraperPort `
    -ArtifactDir (Join-Path "D:\WorkSpace\NPDev\Build\scrapforai-artifacts" "12works-pantry-expiry-tracker")
...
Ok ("Browser verification green across " + $routines.Count + " routine(s) (Item create with nested location object, enum, date field; Flow-emitted custom event consumed by an orchestration rule).")
