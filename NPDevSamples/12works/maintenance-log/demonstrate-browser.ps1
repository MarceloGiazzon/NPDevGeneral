...
# Browser-level demonstration for 12works/maintenance-log: drives the generated
...
# unexpected external requests, screenshots. Exercises Asset create, then
# ServiceEntry create via the search-dialog picker referencing that Asset; the
# create Flow emits a custom event consumed by an orchestration rule that sends
# a notification.
#
# Prereq: the generated app must already be running. Generate + start it first:
#   NPDevSamples/scripts/generate-sample-app.ps1 -SampleId "12works\maintenance-log"
#   NPDevSamples/scripts/run-sample-app.ps1 -SampleId "12works\maintenance-log"
...
# Neither Asset nor ServiceEntry declares a server-side search field, so the UI's filter box
# can't isolate this run's rows from earlier manual/automated test data -- wipe both tables
# first so the routine's assertions are genuinely true on every rerun. ServiceEntry must be
# deleted before Asset since it references Asset.
$existingEntries = @(Invoke-RestMethod -Method Get -Uri ($BaseUrl + "/api/service_entrys") -Headers @{ "X-API-Key" = "dev-key" } | ForEach-Object { $_ })
foreach ($e in $existingEntries) {
    Invoke-RestMethod -Method Delete -Uri ($BaseUrl + "/api/service_entrys/" + $e.id) -Headers @{ "X-API-Key" = "dev-key" } | Out-Null
}
if ($existingEntries.Count -gt 0) { Info ("Cleaned up " + $existingEntries.Count + " pre-existing Service entry row(s) before running.") }

$existingAssets = @(Invoke-RestMethod -Method Get -Uri ($BaseUrl + "/api/assets") -Headers @{ "X-API-Key" = "dev-key" } | ForEach-Object { $_ })
foreach ($a in $existingAssets) {
    Invoke-RestMethod -Method Delete -Uri ($BaseUrl + "/api/assets/" + $a.id) -Headers @{ "X-API-Key" = "dev-key" } | Out-Null
}
if ($existingAssets.Count -gt 0) { Info ("Cleaned up " + $existingAssets.Count + " pre-existing Asset row(s) before running.") }

$samplesRoot = Normalize-AbsolutePath (Join-Path $PSScriptRoot "..\..")
$sample = Resolve-NPDevSample -SamplesRoot $samplesRoot -SampleId "12works\maintenance-log"
...
$sharedVars = @{
    assetName = "UITEST-ASSET-$runStamp"
}
...
$ctx = Start-ScrapForAI -AppBaseUrl $BaseUrl -Root $ScrapForAIRoot -Port $ScraperPort `
    -ArtifactDir (Join-Path "D:\WorkSpace\NPDev\Build\scrapforai-artifacts" "12works-maintenance-log")
...
Ok ("Browser verification green across " + $routines.Count + " routine(s) (Asset create; ServiceEntry create via search-dialog Asset picker; Flow-emitted custom event consumed by an orchestration rule).")
