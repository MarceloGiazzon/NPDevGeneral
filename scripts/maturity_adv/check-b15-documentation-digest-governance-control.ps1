...
$checks = @(
    ...
    (New-MaturityCheck -Name "required-project-digests-current" -Status $(if ($null -ne $requiredProjectDigestsCheck -and [string]$requiredProjectDigestsCheck.status -eq "passed") { "passed" } else { "failed" }) -Expectation "Required PROJECT_DIGEST.md coverage must remain intact." -Summary $(if ($null -ne $requiredProjectDigestsCheck -and [string]$requiredProjectDigestsCheck.status -eq "passed") { "Required project digest coverage remains intact." } else { "One or more required PROJECT_DIGEST.md files are missing." }) -Data @{ requiredProjectDigests = $requiredProjectDigestsCheck })
