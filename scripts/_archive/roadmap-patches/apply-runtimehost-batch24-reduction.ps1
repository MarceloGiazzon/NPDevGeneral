...
$outPath = Join-Path $WorkspaceRoot 'scripts\reports\out\runtimehost-batch24-reduction-report.json'
...
$report = [pscustomobject]@{
    ...
    scriptPath = 'scripts\quality\apply-runtimehost-batch24-reduction.ps1'
...
if ($missing.Count -gt 0) {
    throw "Batch 24 reduction aborted because one or more planned files were missing. See $outPath"
}

if ($DryRun) {
    Write-Host "OK    RuntimeHost Batch 24 dry-run completed. Report: $outPath"
} else {
    Write-Host "OK    RuntimeHost Batch 24 reduction applied. Removed $removedCount file(s). Report: $outPath"
}
