...
if (-not (Test-Path -LiteralPath $PlanPath)) {
    throw "Batch 29 plan file not found: $PlanPath"
}
...
$reportPath = Join-Path $WorkspaceRoot 'scripts\reports\out\runtimehost-batch29-plan-report.json'
$report = [pscustomobject]@{
    generatedAt = (Get-Date).ToString('o')
    scriptPath = 'scripts\tests\Test-RuntimeHostBatch29Plan.ps1'
...
if ($missing.Count -gt 0) {
    throw "Batch 29 plan validation failed. Missing file(s): $($missing -join ', ')"
}

Write-Host "OK    RuntimeHost Batch 29 plan validation passed."
