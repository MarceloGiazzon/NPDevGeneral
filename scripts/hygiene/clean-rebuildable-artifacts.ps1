...
$reportInWorkspace = $false
if ([string]::IsNullOrWhiteSpace($ReportPath)) {
    $ReportPath = Join-Path ([System.IO.Path]::GetTempPath()) ("npdev-rebuildable-clean-report-" + [guid]::NewGuid().ToString("N") + ".json")
}
else {
    $ReportPath = Normalize-NPDevPath $ReportPath
    $reportInWorkspace = $ReportPath.StartsWith($WorkspaceRoot, [System.StringComparison]::OrdinalIgnoreCase)
}

$workspaceCleanupReportPath = Join-Path ([System.IO.Path]::GetTempPath()) ("npdev-workspace-clean-" + [guid]::NewGuid().ToString("N") + ".json")
$workspaceCleanupScript = Resolve-NPDevWorkspacePath $WorkspaceRoot "scripts\hygiene\clean-workspace-state.ps1"
