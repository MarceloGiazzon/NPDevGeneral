...
$scriptPath = 'scripts\quality\run-runtimehost-batch24-verification.ps1'
$outPath = Join-Path $WorkspaceRoot 'scripts\reports\out\runtimehost-batch24-verification-report.json'
$report = New-StructuredRunReport -WorkspaceRoot $WorkspaceRoot -ScriptPath $scriptPath -RunIdPrefix 'runtimehost-batch24-verification'
...
if ($report.overallStatus -eq 'failed') {
    Write-Warning 'RuntimeHost Batch 24 verification failed.'
    throw 'RuntimeHost Batch 24 verification failed.'
}

Write-Host 'OK    RuntimeHost Batch 24 verification completed.'
