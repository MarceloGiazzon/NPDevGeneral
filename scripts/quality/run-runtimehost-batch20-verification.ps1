...
$scriptPath = 'scripts\quality\run-runtimehost-batch20-verification.ps1'
$outPath = Join-Path $WorkspaceRoot 'scripts\reports\out\runtimehost-batch20-verification-report.json'
$report = New-StructuredRunReport -WorkspaceRoot $WorkspaceRoot -ScriptPath $scriptPath -RunIdPrefix 'runtimehost-batch20-verification'
...
if ($report.overallStatus -eq 'failed') {
    Write-Warning 'RuntimeHost Batch 20 verification failed.'
    throw 'RuntimeHost Batch 20 verification failed.'
}

Write-Host 'OK    RuntimeHost Batch 20 verification completed.'
