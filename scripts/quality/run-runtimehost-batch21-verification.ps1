...
$scriptPath = 'scripts\quality\run-runtimehost-batch21-verification.ps1'
$outPath = Join-Path $WorkspaceRoot 'scripts\reports\out\runtimehost-batch21-verification-report.json'
$report = New-StructuredRunReport -WorkspaceRoot $WorkspaceRoot -ScriptPath $scriptPath -RunIdPrefix 'runtimehost-batch21-verification'
...
if ($report.overallStatus -eq 'failed') {
    Write-Warning 'RuntimeHost Batch 21 verification failed.'
    throw 'RuntimeHost Batch 21 verification failed.'
}

Write-Host 'OK    RuntimeHost Batch 21 verification completed.'
