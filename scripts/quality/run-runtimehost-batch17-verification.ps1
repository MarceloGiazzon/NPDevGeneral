...
$scriptPath = 'scripts\quality\run-runtimehost-batch17-verification.ps1'
$outPath = Join-Path $WorkspaceRoot 'scripts\reports\out\runtimehost-batch17-verification-report.json'
$report = New-StructuredRunReport -WorkspaceRoot $WorkspaceRoot -ScriptPath $scriptPath -RunIdPrefix 'runtimehost-batch17-verification'
...
if ($report.overallStatus -eq 'failed') {
    Write-Warning 'RuntimeHost Batch 17 verification failed.'
    throw 'RuntimeHost Batch 17 verification failed.'
}

Write-Host 'OK    RuntimeHost Batch 17 verification completed.'
