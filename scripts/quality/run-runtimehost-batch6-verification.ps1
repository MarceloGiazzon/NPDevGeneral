...
$scriptPath = 'scripts\quality\run-runtimehost-batch6-verification.ps1'
$outPath = Join-Path $WorkspaceRoot 'scripts\reports\out\runtimehost-batch6-verification-report.json'
$report = New-StructuredRunReport -WorkspaceRoot $WorkspaceRoot -ScriptPath $scriptPath -RunIdPrefix 'runtimehost-batch6-verification'
...
if ($report.overallStatus -eq 'failed') {
    Write-Warning 'RuntimeHost Batch 6 verification failed.'
    throw 'RuntimeHost Batch 6 verification failed.'
}

Write-Host 'OK    RuntimeHost Batch 6 verification completed.'
