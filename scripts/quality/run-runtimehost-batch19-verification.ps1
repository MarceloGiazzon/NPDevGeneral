...
$scriptPath = 'scripts\quality\run-runtimehost-batch19-verification.ps1'
$outPath = Join-Path $WorkspaceRoot 'scripts\reports\out\runtimehost-batch19-verification-report.json'
$report = New-StructuredRunReport -WorkspaceRoot $WorkspaceRoot -ScriptPath $scriptPath -RunIdPrefix 'runtimehost-batch19-verification'
...
if ($report.overallStatus -eq 'failed') {
    Write-Warning 'RuntimeHost Batch 19 verification failed.'
    throw 'RuntimeHost Batch 19 verification failed.'
}

Write-Host 'OK    RuntimeHost Batch 19 verification completed.'
