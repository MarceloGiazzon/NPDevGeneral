...
$scriptPath = 'scripts\quality\run-runtimehost-batch12-verification.ps1'
$outPath = Join-Path $WorkspaceRoot 'scripts\reports\out\runtimehost-batch12-verification-report.json'
$report = New-StructuredRunReport -WorkspaceRoot $WorkspaceRoot -ScriptPath $scriptPath -RunIdPrefix 'runtimehost-batch12-verification'
...
if ($report.overallStatus -eq 'failed') {
    Write-Warning 'RuntimeHost Batch 12 verification failed.'
    throw 'RuntimeHost Batch 12 verification failed.'
}

Write-Host 'OK    RuntimeHost Batch 12 verification completed.'
