...
$scriptPath = 'scripts\quality\run-runtimehost-batch15-verification.ps1'
$outPath = Join-Path $WorkspaceRoot 'scripts\reports\out\runtimehost-batch15-verification-report.json'
$report = New-StructuredRunReport -WorkspaceRoot $WorkspaceRoot -ScriptPath $scriptPath -RunIdPrefix 'runtimehost-batch15-verification'
...
if ($report.overallStatus -eq 'failed') {
    Write-Warning 'RuntimeHost Batch 15 verification failed.'
    throw 'RuntimeHost Batch 15 verification failed.'
}

Write-Host 'OK    RuntimeHost Batch 15 verification completed.'
