...
$scriptPath = 'scripts\quality\run-runtimehost-batch13-verification.ps1'
$outPath = Join-Path $WorkspaceRoot 'scripts\reports\out\runtimehost-batch13-verification-report.json'
$report = New-StructuredRunReport -WorkspaceRoot $WorkspaceRoot -ScriptPath $scriptPath -RunIdPrefix 'runtimehost-batch13-verification'
...
if ($report.overallStatus -eq 'failed') {
    Write-Warning 'RuntimeHost Batch 13 verification failed.'
    throw 'RuntimeHost Batch 13 verification failed.'
}

Write-Host 'OK    RuntimeHost Batch 13 verification completed.'
