...
$scriptPath = 'scripts\quality\run-runtimehost-batch7-verification.ps1'
$outPath = Join-Path $WorkspaceRoot 'scripts\reports\out\runtimehost-batch7-verification-report.json'
$report = New-StructuredRunReport -WorkspaceRoot $WorkspaceRoot -ScriptPath $scriptPath -RunIdPrefix 'runtimehost-batch7-verification'
...
if ($report.overallStatus -eq 'failed') {
    Write-Warning 'RuntimeHost Batch 7 verification failed.'
    throw 'RuntimeHost Batch 7 verification failed.'
}

Write-Host 'OK    RuntimeHost Batch 7 verification completed.'
