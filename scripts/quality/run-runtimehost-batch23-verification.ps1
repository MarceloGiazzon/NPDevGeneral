...
$scriptPath = 'scripts\quality\run-runtimehost-batch23-verification.ps1'
$outPath = Join-Path $WorkspaceRoot 'scripts\reports\out\runtimehost-batch23-verification-report.json'
$report = New-StructuredRunReport -WorkspaceRoot $WorkspaceRoot -ScriptPath $scriptPath -RunIdPrefix 'runtimehost-batch23-verification'
...
if ($report.overallStatus -eq 'failed') {
    Write-Warning 'RuntimeHost Batch 23 verification failed.'
    throw 'RuntimeHost Batch 23 verification failed.'
}

Write-Host 'OK    RuntimeHost Batch 23 verification completed.'
