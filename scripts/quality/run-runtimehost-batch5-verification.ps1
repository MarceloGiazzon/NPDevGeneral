...
$scriptPath = 'scripts\quality\run-runtimehost-batch5-verification.ps1'
$outPath = Join-Path $WorkspaceRoot 'scripts\reports\out\runtimehost-batch5-verification-report.json'
$report = New-StructuredRunReport -WorkspaceRoot $WorkspaceRoot -ScriptPath $scriptPath -RunIdPrefix 'runtimehost-batch5-verification'
...
if ($report.overallStatus -eq 'failed') {
    Write-Warning 'RuntimeHost Batch 5 verification failed.'
    throw 'RuntimeHost Batch 5 verification failed.'
}

Write-Host 'OK    RuntimeHost Batch 5 verification completed.'
