...
$scriptPath = 'scripts\quality\run-runtimehost-batch25-verification.ps1'
$outPath = Join-Path $WorkspaceRoot 'scripts\reports\out\runtimehost-batch25-verification-report.json'
$report = New-StructuredRunReport -WorkspaceRoot $WorkspaceRoot -ScriptPath $scriptPath -RunIdPrefix 'runtimehost-batch25-verification'
...
if ($report.overallStatus -eq 'failed') {
    Write-Warning 'RuntimeHost Batch 25 verification failed.'
    throw 'RuntimeHost Batch 25 verification failed.'
}

Write-Host 'OK    RuntimeHost Batch 25 verification completed.'
