...
$scriptPath = 'scripts\quality\run-runtimehost-batch14-verification.ps1'
$outPath = Join-Path $WorkspaceRoot 'scripts\reports\out\runtimehost-batch14-verification-report.json'
$report = New-StructuredRunReport -WorkspaceRoot $WorkspaceRoot -ScriptPath $scriptPath -RunIdPrefix 'runtimehost-batch14-verification'
...
if ($report.overallStatus -eq 'failed') {
    Write-Warning 'RuntimeHost Batch 14 verification failed.'
    throw 'RuntimeHost Batch 14 verification failed.'
}

Write-Host 'OK    RuntimeHost Batch 14 verification completed.'
