...
$scriptPath = 'scripts\quality\run-runtimehost-batch18-verification.ps1'
$outPath = Join-Path $WorkspaceRoot 'scripts\reports\out\runtimehost-batch18-verification-report.json'
$report = New-StructuredRunReport -WorkspaceRoot $WorkspaceRoot -ScriptPath $scriptPath -RunIdPrefix 'runtimehost-batch18-verification'
...
if ($report.overallStatus -eq 'failed') {
    Write-Warning 'RuntimeHost Batch 18 verification failed.'
    throw 'RuntimeHost Batch 18 verification failed.'
}

Write-Host 'OK    RuntimeHost Batch 18 verification completed.'
