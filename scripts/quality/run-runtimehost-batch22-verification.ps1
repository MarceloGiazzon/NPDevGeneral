...
$scriptPath = 'scripts\quality\run-runtimehost-batch22-verification.ps1'
$outPath = Join-Path $WorkspaceRoot 'scripts\reports\out\runtimehost-batch22-verification-report.json'
$report = New-StructuredRunReport -WorkspaceRoot $WorkspaceRoot -ScriptPath $scriptPath -RunIdPrefix 'runtimehost-batch22-verification'
...
if ($report.overallStatus -eq 'failed') {
    Write-Warning 'RuntimeHost Batch 22 verification failed.'
    throw 'RuntimeHost Batch 22 verification failed.'
}

Write-Host 'OK    RuntimeHost Batch 22 verification completed.'
