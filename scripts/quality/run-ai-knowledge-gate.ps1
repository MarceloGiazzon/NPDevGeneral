...
try {
    ...
    # [1/8] Register self-check runs FIRST: platform-status.json is DERIVED from the same documents,
    ...
    Write-Host "[1/8] Checking register/roadmap summary rows against their detail sections..."
    ...
    if ($Fix) {
        Write-Host "[2/8] Regenerating platform-status projection..." -ForegroundColor Yellow
        & $py "scripts/ai/extract_platform_status.py"
        if ($LASTEXITCODE -ne 0) { $failures += "platform-status regeneration failed" }
    } else {
        Write-Host "[2/8] Checking platform-status projection is current..."
    ...
    # [5/8] Same question as [1/8], asked of a different mechanism: is this quality tool still doing
    ...
    Write-Host "[5/8] Checking the security pattern sweep still catches its known bugs..."
    ...
    # [6/8] Run the sweep against the CODEBASE and fail on anything untriaged. Until now the gate
    # proved the detector worked ([5/8]) but never actually pointed it at the repo, so a new hit could
    ...
    Write-Host "[6/8] Checking for untriaged security-pattern hits..."
    ...
    # [7/8] Report-only (Phase 2.4, docs/REMAINDER_CLOSURE_PLAN.md): calibrated against both real
    ...
    Write-Host "[7/8] Checking for narrative-status drift (report-only)..."
    & $py "scripts/quality/check-narrative-status-drift.py"

    # [8/8] T1.3: a custom Gradle Test task (behaviorTest/integrationTest/a future contractTest) that
    # is declared but reachable from no CI workflow is a test that only runs on one laptop -- exactly
    # what REG-49's residual behaviorTest was until T1.2 wired it in. Blocking, same rationale as [6/8].
    Write-Host "[8/8] Checking every custom Gradle Test task is reachable from a CI workflow..."
