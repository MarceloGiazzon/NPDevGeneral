...
try {
    ...
    # [1/35] Register self-check runs FIRST: platform-status.json is DERIVED from the same documents,
    ...
    Write-Host "[1/35] Checking register/roadmap summary rows against their detail sections..."
    ...
    if ($Fix) {
        Write-Host "[2/35] Regenerating platform-status projection..." -ForegroundColor Yellow
        & $py "scripts/ai/extract_platform_status.py"
        if ($LASTEXITCODE -ne 0) { $failures += "platform-status regeneration failed" }
    } else {
        Write-Host "[2/35] Checking platform-status projection is current..."
    ...
    # [5/35] Same question as [1/35], asked of a different mechanism: is this quality tool still doing
    ...
    Write-Host "[5/35] Checking the security pattern sweep still catches its known bugs..."
    ...
    # [6/35] Run the sweep against the CODEBASE and fail on anything untriaged. Until now the gate
    # proved the detector worked ([5/35]) but never actually pointed it at the repo, so a new hit could
    ...
    Write-Host "[6/35] Checking for untriaged security-pattern hits..."
    ...
    # [7/35] Report-only (Phase 2.4, docs/archive/programme-history/REMAINDER_CLOSURE_PLAN.md): calibrated against both real
    ...
    Write-Host "[7/35] Checking for narrative-status drift (report-only)..."
    & $py "scripts/quality/check-narrative-status-drift.py"

    # [8/35] T1.3: a custom Gradle Test task (behaviorTest/integrationTest/a future contractTest) that
    # is declared but reachable from no CI workflow is a test that only runs on one laptop -- exactly
    # what REG-49's residual behaviorTest was until T1.2 wired it in. Blocking, same rationale as [6/35].
    Write-Host "[8/35] Checking every custom Gradle Test task is reachable from a CI workflow..."
    ...
    # [9/35] 2.A.2 (docs/DSL2_AND_DECOMPOSITION_PLAN.md): model.schema.json is duplicated in four
    # places with nothing previously enforcing they stay in sync -- they had already drifted by the
    # time this gate was written. Blocking, same rationale as [6/35] and [8/35]: an unsynced schema
    ...
    Write-Host "[9/35] Checking the four model.schema.json copies are still semantically identical..."
    ...
    # [10/35] R-G1 static half (docs/REMEDIATION_PLAN.md): the panel-provenance impact gate (F4)
    ...
    Write-Host "[10/35] Checking *.panel.json manifests structurally validate (no live app needed)..."
    ...
    # [11/35] C4 (docs/CORPUS_INTEGRITY_PLAN.md): every model.json under AppGen/apps + NPDevSamples
    # must still parse against the REAL validator. Blocking, same rationale as [6/35]/[8/35]/[9/35] --
    ...
    Write-Host "[11/35] Checking every corpus model.json still parses..."
    ...
    Write-Host "[12/35] Checking every relative markdown link resolves..."
    ...
    # [13/35] Found by accident verifying F1/F2 on a live PR, 2026-07-29: an unquoted colon inside a
    ...
    Write-Host "[13/35] Checking every workflow file is valid YAML..."
    ...
    # [14/35] F6 (docs/FINAL_OPEN_ITEMS_PLAN.md): the simple-user-registry-* and p77-hookproof(-pg)
    ...
    Write-Host "[14/35] Checking engine-variant families stay byte-identical..."
    ...
    # [15/35] F8 (docs/FINAL_OPEN_ITEMS_PLAN.md): the corpus-parse gate (check 11) answers "does
    ...
    Write-Host "[15/35] Checking every DSL feature has at least one corpus model exercising it..."
    ...
    # [16/35] docs/RECORD_SURFACES_PLAN.md P4: two mechanical record-surface claims that go stale
    ...
    # rationale as [6/35]/[8/35]/[9/35]/[11/35]/[12/35]/[13/35].
    Write-Host "[16/35] Checking branch freshness vs. origin/main and CLAUDE.md's large-file size claims..."
    ...
    # [17/35] docs/FAIL_OPEN_PLAN.md R3: three allowlists (corpus-parse, test-task-coverage,
    ...
    Write-Host "[17/35] Checking allowlist entries carry a REG/B citation, reporting allowlist sizes..."
    ...
    # [18/35] docs/INVOCATION_TOPOLOGY_PLAN.md T1: a calibration nobody runs is a claim, not
    ...
    Write-Host "[18/35] Running every --calibrate self-test (list derived from argparse, not hand-written)..."
    ...
    # [19/35] O4 (Move 11 W2): the EXISTENCE half of the panel-provenance impact gate. Its only
    ...
    # printed PASS, same convention as [10/35].
    Write-Host "[19/35] Checking panel-provenance manifests against each built app's current model..."
    ...
    # [20/35] O4 (Move 11 W2): the emit-side mirror of [15/35]. Also invoked by npdev-pr-gate.yml
    ...
    Write-Host "[20/35] Checking every asserted DSL feature survives GENERATION, not just parsing..."
    ...
    # [21/35] O5 (Move 11 W4): does every step type declared in model.schema.json have a MODEL-level
    ...
    Write-Host "[21/35] Checking every step type has a model-level validation test..."
    ...
    # [22/35] Move 13 P6: makes docs/X0_SILENT_EXPRESSION_REGISTER.md's discipline a permanent gate
    ...
    Write-Host "[22/35] Checking the X0 silent-answer register against its machine-checked registry..."
    ...
    # [23/35] Fast Lane plan Sec.7.4 (2026-08-01): "a check absent from verification-cadence.json
    ...
    Write-Host "[23/35] Checking every run-all-gates.ps1 gate has a verification-cadence.json staleness deadline..."
    ...
    # [24/35] REG-110 (Move 14 Phase D): NPDevCli/tests/test_acceptance_runner.py -- the acceptance
    ...
    Write-Host "[24/35] Checking NPDevCli's own test suite (acceptance runner + dsl-2 migration)..."
    ...
    # [25/35] Move 14 Phase E item E1 (U2): "a rule applied in one place, not mirrored to its twin"
    ...
    Write-Host "[25/35] Checking registered twin-pair rules (REG-89/104/112's family) haven't diverged..."
    ...
    # [26/35] Move 15 Phase D item D1: five separate times a console/screen record said "blocked by
    ...
    Write-Host "[26/35] Checking cited blockers (REG-nn) in checklist/findings docs are still open..."
    ...
    # [27/35] S4 Phase B (S4_SPEC.md): does an ADR decision the owner accepted actually have live
    ...
    Write-Host "[27/35] Checking accepted ADR decisions carrying a decision-check claim are implemented..."
    ...
    # [28/35] storage/PLAN.md S1: the 41 dialect-bound SQL sites this repo used to carry inline were
    ...
    Write-Host "[28/35] Checking no dialect-bound SQL sits outside the dialect package..."
    ...
    # [29/35] storage/FULL_SUPPORT_PLAN.md W1.1: the storage conformance suite must run on DIGESTS,
    ...
    Write-Host "[29/35] Checking conformance container images are pinned to digests..."
    ...
    # [30/35] storage/FULL_SUPPORT_PLAN.md W3, and the mechanical half of STOR-2. A storage message
    ...
    Write-Host "[30/35] Checking no storage message claims a rollback without asking the dialect..."
    ...
    # [31/35] storage/FULL_SUPPORT_PLAN.md W6.1. Every config.json carries a $schema pointer to
    ...
    Write-Host "[31/35] Validating every corpus config.json against config.schema.json..."
    ...
    # [32/35] storage/OPEN_ITEMS_PLAN.md W7 (ledger STOR-5): the guarded-DDL idioms NPDev emits must
    # be ones the target engine can actually run. check-dialect-sites.py [29/35] guards the SOURCE;
    ...
    Write-Host "[32/35] Checking emitted SQL is portable to the engine each script was generated for..."
    ...
    # clean today.
    Write-Host "[33/35] Checking every engine gets the same environment toolbox..."
    ...
    Write-Host "[34/35] Checking no template resource shadows a generated one..."
    ...
    # [35/35] docs/INVOCATION_TOPOLOGY_PLAN.md T2: every script under scripts/ must declare BOTH a
    ...
    Write-Host "[35/35] Checking every script declares a classification + invocation matching reality..."
