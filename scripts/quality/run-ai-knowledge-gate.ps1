...
try {
    ...
    # [1/13] Register self-check runs FIRST: platform-status.json is DERIVED from the same documents,
    ...
    Write-Host "[1/13] Checking register/roadmap summary rows against their detail sections..."
    ...
    if ($Fix) {
        Write-Host "[2/13] Regenerating platform-status projection..." -ForegroundColor Yellow
        & $py "scripts/ai/extract_platform_status.py"
        if ($LASTEXITCODE -ne 0) { $failures += "platform-status regeneration failed" }
    } else {
        Write-Host "[2/13] Checking platform-status projection is current..."
    ...
    # [5/13] Same question as [1/13], asked of a different mechanism: is this quality tool still doing
    ...
    Write-Host "[5/13] Checking the security pattern sweep still catches its known bugs..."
    ...
    # [6/13] Run the sweep against the CODEBASE and fail on anything untriaged. Until now the gate
    # proved the detector worked ([5/13]) but never actually pointed it at the repo, so a new hit could
    ...
    Write-Host "[6/13] Checking for untriaged security-pattern hits..."
    ...
    # [7/13] Report-only (Phase 2.4, docs/archive/programme-history/REMAINDER_CLOSURE_PLAN.md): calibrated against both real
    ...
    Write-Host "[7/13] Checking for narrative-status drift (report-only)..."
    & $py "scripts/quality/check-narrative-status-drift.py"

    # [8/13] T1.3: a custom Gradle Test task (behaviorTest/integrationTest/a future contractTest) that
    # is declared but reachable from no CI workflow is a test that only runs on one laptop -- exactly
    # what REG-49's residual behaviorTest was until T1.2 wired it in. Blocking, same rationale as [6/13].
    Write-Host "[8/13] Checking every custom Gradle Test task is reachable from a CI workflow..."
    ...
    # [9/13] 2.A.2 (docs/DSL2_AND_DECOMPOSITION_PLAN.md): model.schema.json is duplicated in four
    # places with nothing previously enforcing they stay in sync -- they had already drifted by the
    # time this gate was written. Blocking, same rationale as [6/13] and [8/13]: an unsynced schema
    ...
    Write-Host "[9/13] Checking the four model.schema.json copies are still semantically identical..."
    ...
    # [10/13] R-G1 static half (docs/REMEDIATION_PLAN.md): the panel-provenance impact gate (F4)
    ...
    Write-Host "[10/13] Checking *.panel.json manifests structurally validate (no live app needed)..."
    ...
    # [11/13] C4 (docs/CORPUS_INTEGRITY_PLAN.md): every model.json under AppGen/apps + NPDevSamples
    # must still parse against the REAL validator. Blocking, same rationale as [6/13]/[8/13]/[9/13] --
    ...
    Write-Host "[11/13] Checking every corpus model.json still parses..."
    ...
    Write-Host "[12/13] Checking every relative markdown link resolves..."
    ...
    # [13/13] Found by accident verifying F1/F2 on a live PR, 2026-07-29: an unquoted colon inside a
    ...
    # 11/13 and 12/13. Syntax only (yaml.safe_load), not GitHub Actions schema validation -- cheapest
    # version of the fix, matching the failure mode that actually happened.
    Write-Host "[13/13] Checking every workflow file is valid YAML..."
