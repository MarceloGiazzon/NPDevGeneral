...
.DESCRIPTION
    ...
    check-blocker-citation-freshness.py (see [37/43] below). All 177 items the first guarded were
...
try {
    ...
    if ($Fix) {
        Write-Host "[1/43] Regenerating gaps roadmap + platform-status projection..." -ForegroundColor Yellow
        ...
        Write-Host "[1/43] Checking gaps roadmap generates + platform-status projection is current..."
    ...
    Write-Host "[2/43] Validating knowledge cards..."
    & $py "scripts/ai/build_knowledge.py" --validate-only
    if ($LASTEXITCODE -ne 0) { $failures += "knowledge-card validation failed" }

    # Bundled into the same [2/43] slot (docs-decoupling-2026-08-11 PLAN.md Phase 1's precedent:
    ...
    Write-Host "[3/43] Checking failure-signature normalizer..."
    ...
    # [4/43] The same question check-register-consistency.py (deleted, md-zero-2026-08-11 PLAN.md
    ...
    Write-Host "[4/43] Checking the security pattern sweep still catches its known bugs..."
    ...
    # [5/43] Run the sweep against the CODEBASE and fail on anything untriaged. Until now the gate
    # proved the detector worked ([4/43]) but never actually pointed it at the repo, so a new hit could
    ...
    Write-Host "[5/43] Checking for untriaged security-pattern hits..."
    ...
    # [6/43] T1.3: a custom Gradle Test task (behaviorTest/integrationTest/a future contractTest) that
    # is declared but reachable from no CI workflow is a test that only runs on one laptop -- exactly
    # what REG-49's residual behaviorTest was until T1.2 wired it in. Blocking, same rationale as [5/43].
    Write-Host "[6/43] Checking every custom Gradle Test task is reachable from a CI workflow..."
    ...
    # [7/43] 2.A.2 (docs/DSL2_AND_DECOMPOSITION_PLAN.md): model.schema.json is duplicated in four
    # places with nothing previously enforcing they stay in sync -- they had already drifted by the
    # time this gate was written. Blocking, same rationale as [5/43] and [6/43]: an unsynced schema
    ...
    Write-Host "[7/43] Checking the four model.schema.json copies are still semantically identical..."
    ...
    # [8/43] R-G1 static half (docs/REMEDIATION_PLAN.md): the panel-provenance impact gate (F4)
    ...
    Write-Host "[8/43] Checking *.panel.json manifests structurally validate (no live app needed)..."
    ...
    # [9/43] C4 (docs/CORPUS_INTEGRITY_PLAN.md): every model.json under AppGen/apps + NPDevSamples
    # must still parse against the REAL validator. Blocking, same rationale as [5/43]/[6/43]/[7/43] --
    ...
    Write-Host "[9/43] Checking every corpus model.json still parses..."
    ...
    Write-Host "[10/43] Checking every relative markdown link resolves..."
    ...
    # [11/43] Found by accident verifying F1/F2 on a live PR, 2026-07-29: an unquoted colon inside a
    ...
    Write-Host "[11/43] Checking every workflow file is valid YAML..."
    ...
    # [12/43] F6 (docs/FINAL_OPEN_ITEMS_PLAN.md): the simple-user-registry-* and p77-hookproof(-pg)
    ...
    Write-Host "[12/43] Checking engine-variant families stay byte-identical..."
    ...
    # [13/43] F8 (docs/FINAL_OPEN_ITEMS_PLAN.md): the corpus-parse gate (check 11) answers "does
    ...
    Write-Host "[13/43] Checking every DSL feature has at least one corpus model exercising it..."
    ...
    # [14/43] docs/RECORD_SURFACES_PLAN.md P4: two mechanical record-surface claims that go stale
    ...
    # either. Blocking, same rationale as [5/43]/[6/43]/[7/43]/[9/43]/[10/43]/[11/43].
    Write-Host "[14/43] Checking branch freshness vs. origin/main and record-surfaces.json's large-file size claims..."
    ...
    # [15/43] docs/FAIL_OPEN_PLAN.md R3: three allowlists (corpus-parse, test-task-coverage,
    ...
    Write-Host "[15/43] Checking allowlist entries carry a REG/B citation, reporting allowlist sizes..."
    ...
    # [16/43] docs/INVOCATION_TOPOLOGY_PLAN.md T1: a calibration nobody runs is a claim, not
    ...
    Write-Host "[16/43] Running every --calibrate self-test (list derived from argparse, not hand-written)..."
    ...
    # [17/43] O4 (Move 11 W2): the EXISTENCE half of the panel-provenance impact gate. Its only
    ...
    # printed PASS, same convention as [8/43].
    Write-Host "[17/43] Checking panel-provenance manifests against each built app's current model..."
    ...
    # [18/43] O4 (Move 11 W2): the emit-side mirror of [13/43]. Also invoked by npdev-pr-gate.yml
    ...
    Write-Host "[18/43] Checking every asserted DSL feature survives GENERATION, not just parsing..."
    ...
    # [19/43] O5 (Move 11 W4): does every step type declared in model.schema.json have a MODEL-level
    ...
    Write-Host "[19/43] Checking every step type has a model-level validation test..."
    ...
    # [20/43] Move 13 P6: makes the X0 silent-answer discipline a permanent gate instead of something
    ...
    Write-Host "[20/43] Checking the X0 silent-answer registry..."
    ...
    # [21/43] Fast Lane plan Sec.7.4 (2026-08-01): "a check absent from verification-cadence.json
    ...
    # becoming a visible, blocking overdue state. Must run before script-inventory-check ([41/43]
    # below), for the same reason: that check verifies check-cadence-coverage.py itself is invoked
    # by something.
    Write-Host "[21/43] Checking every run-all-gates.ps1 gate has a verification-cadence.json staleness deadline..."
    ...
    # [22/43] REG-110 (Move 14 Phase D): NPDevCli/tests/test_acceptance_runner.py -- the acceptance
    ...
    Write-Host "[22/43] Checking NPDevCli's own test suite (acceptance runner + dsl-2 migration)..."
    ...
    # [23/43] Move 14 Phase E item E1 (U2): "a rule applied in one place, not mirrored to its twin"
    ...
    Write-Host "[23/43] Checking registered twin-pair rules (REG-89/104/112's family) haven't diverged..."
    ...
    # [24/43] S4 Phase B (S4_SPEC.md): does an ADR decision the owner accepted actually have live
    ...
    Write-Host "[24/43] Checking accepted ADR decisions carrying a decision-check claim are implemented..."
    ...
    # [25/43] storage/PLAN.md S1: the 41 dialect-bound SQL sites this repo used to carry inline were
    ...
    Write-Host "[25/43] Checking no dialect-bound SQL sits outside the dialect package..."
    ...
    # [26/43] storage/FULL_SUPPORT_PLAN.md W1.1: the storage conformance suite must run on DIGESTS,
    ...
    Write-Host "[26/43] Checking conformance container images are pinned to digests..."
    ...
    # [27/43] storage/FULL_SUPPORT_PLAN.md W3, and the mechanical half of STOR-2. A storage message
    ...
    Write-Host "[27/43] Checking no storage message claims a rollback without asking the dialect..."
    ...
    # [28/43] storage/FULL_SUPPORT_PLAN.md W6.1. Every config.json carries a $schema pointer to
    ...
    Write-Host "[28/43] Validating every corpus config.json against config.schema.json..."
    ...
    # [29/43] storage/OPEN_ITEMS_PLAN.md W7 (ledger STOR-5): the guarded-DDL idioms NPDev emits must
    # be ones the target engine can actually run. check-dialect-sites.py [26/43] guards the SOURCE;
    ...
    Write-Host "[29/43] Checking emitted SQL is portable to the engine each script was generated for..."
    ...
    # clean today.
    Write-Host "[30/43] Checking every engine gets the same environment toolbox..."
    ...
    Write-Host "[31/43] Checking no template resource shadows a generated one..."
    ...
    # [32/43] The mirror of [25/43]: that one fails when SQL is written OUTSIDE the dialect package,
    ...
    Write-Host "[32/43] Checking every SqlDialect answer has a caller that asks it..."
    ...
    # [33/43] PRE_ROUND_FIXES.md section 4: the STATIC half of what the first-run harness proves.
    ...
    Write-Host "[33/43] Checking every documented command/default a reader would copy..."
    ...
    # [34/43] QUAL-2 / round3 Part 2.1: a Files.list|walk|find|lines|newDirectoryStream whose Stream
    ...
    # very line.
    Write-Host "[34/43] Checking every Files stream is closed (try-with-resources)..."
    ...
    # [35/43] FOUR_AND_EXTERNAL.md A.1: the first-run harness's own command extractor, against a
    ...
    Write-Host "[35/43] Checking the first-run harness's command extractor against its corpus..."
    ...
    # [36/43] Any file declaring a shebang must have LF line endings. `core.autocrlf=true` is Git
    ...
    Write-Host "[36/43] Checking every shebang'd file has LF line endings..."
    ...
    # [37/43] The direction check-blocker-citation-freshness.py (deleted, md-zero-2026-08-11 PLAN.md
    ...
    # and [34/43] independently proved the family gone across 1464 Java files. That matters because
    ...
    Write-Host "[37/43] Checking no OPEN ledger item has a fix that already landed..."
    ...
    # [38/43] A download link in an install guide must not name a version. Found twice on 2026-08-10,
    ...
    Write-Host "[38/43] Checking no install guide pins a specific release in a download link..."
    ...
    # [39/43] PORT-1's CLASS, which fixing its instance did not close. The committed React bundle
    ...
    Write-Host "[39/43] Checking committed generated artefacts are not older than their source..."
    ...
    # [40/43] MONITOR_PLAN A3. The routine vocabulary is the ENGINE's, pinned into
    ...
    # looked at. Self-tests first, for the same reason check-engine-parity.py does [30/43]: a
    # checker that can no longer tell a broken routine from a good one has a meaningless verdict.
    Write-Host "[40/43] Checking every checked-in browser routine conforms to the pinned engine schema..."
    ...
    # [41/43] docs/INVOCATION_TOPOLOGY_PLAN.md T2: every script under scripts/ must declare BOTH a
    ...
    Write-Host "[41/43] Checking every script declares a classification + invocation matching reality..."
    ...
    # [42/43] Markdown policy: the process-document ban, and (md-zero-2026-08-11 PLAN.md Phase 7,
    ...
    # [5/43]/[6/43]/[7/43].
    Write-Host "[42/43] Checking no new process document (plan/checklist/findings/register) entered the repo..."
    ...
    # [43/43] PK-4 Stage A/B (the diff engine + the publish gate): does a branch's own change to a
    ...
    Write-Host "[43/43] Checking every built-in pack's own branch-relative change has a correctly-sized version bump..."
