...
try {
    ...
    # [1/29] Register self-check runs FIRST: platform-status.json is DERIVED from the same documents,
    ...
    Write-Host "[1/29] Checking register/roadmap summary rows against their detail sections..."
    ...
    if ($Fix) {
        Write-Host "[2/29] Regenerating platform-status projection..." -ForegroundColor Yellow
        & $py "scripts/ai/extract_platform_status.py"
        if ($LASTEXITCODE -ne 0) { $failures += "platform-status regeneration failed" }
    } else {
        Write-Host "[2/29] Checking platform-status projection is current..."
    ...
    # [5/29] Same question as [1/29], asked of a different mechanism: is this quality tool still doing
    ...
    Write-Host "[5/29] Checking the security pattern sweep still catches its known bugs..."
    ...
    # [6/29] Run the sweep against the CODEBASE and fail on anything untriaged. Until now the gate
    # proved the detector worked ([5/29]) but never actually pointed it at the repo, so a new hit could
    ...
    Write-Host "[6/29] Checking for untriaged security-pattern hits..."
    ...
    # [7/29] Report-only (Phase 2.4, docs/archive/programme-history/REMAINDER_CLOSURE_PLAN.md): calibrated against both real
    ...
    Write-Host "[7/29] Checking for narrative-status drift (report-only)..."
    & $py "scripts/quality/check-narrative-status-drift.py"

    # [8/29] T1.3: a custom Gradle Test task (behaviorTest/integrationTest/a future contractTest) that
    # is declared but reachable from no CI workflow is a test that only runs on one laptop -- exactly
    # what REG-49's residual behaviorTest was until T1.2 wired it in. Blocking, same rationale as [6/29].
    Write-Host "[8/29] Checking every custom Gradle Test task is reachable from a CI workflow..."
    ...
    # [9/29] 2.A.2 (docs/DSL2_AND_DECOMPOSITION_PLAN.md): model.schema.json is duplicated in four
    # places with nothing previously enforcing they stay in sync -- they had already drifted by the
    # time this gate was written. Blocking, same rationale as [6/29] and [8/29]: an unsynced schema
    ...
    Write-Host "[9/29] Checking the four model.schema.json copies are still semantically identical..."
    ...
    # [10/29] R-G1 static half (docs/REMEDIATION_PLAN.md): the panel-provenance impact gate (F4)
    ...
    Write-Host "[10/29] Checking *.panel.json manifests structurally validate (no live app needed)..."
    ...
    # [11/29] C4 (docs/CORPUS_INTEGRITY_PLAN.md): every model.json under AppGen/apps + NPDevSamples
    # must still parse against the REAL validator. Blocking, same rationale as [6/29]/[8/29]/[9/29] --
    ...
    Write-Host "[11/29] Checking every corpus model.json still parses..."
    ...
    Write-Host "[12/29] Checking every relative markdown link resolves..."
    ...
    # [13/29] Found by accident verifying F1/F2 on a live PR, 2026-07-29: an unquoted colon inside a
    ...
    Write-Host "[13/29] Checking every workflow file is valid YAML..."
    ...
    # [14/29] F6 (docs/FINAL_OPEN_ITEMS_PLAN.md): the simple-user-registry-* and p77-hookproof(-pg)
    ...
    Write-Host "[14/29] Checking engine-variant families stay byte-identical..."
    ...
    # [15/29] F8 (docs/FINAL_OPEN_ITEMS_PLAN.md): the corpus-parse gate (check 11) answers "does
    ...
    Write-Host "[15/29] Checking every DSL feature has at least one corpus model exercising it..."
    ...
    # [16/29] docs/RECORD_SURFACES_PLAN.md P4: two mechanical record-surface claims that go stale
    ...
    # rationale as [6/29]/[8/29]/[9/29]/[11/29]/[12/29]/[13/29].
    Write-Host "[16/29] Checking branch freshness vs. origin/main and CLAUDE.md's large-file size claims..."
    ...
    # [17/29] docs/FAIL_OPEN_PLAN.md R3: three allowlists (corpus-parse, test-task-coverage,
    ...
    Write-Host "[17/29] Checking allowlist entries carry a REG/B citation, reporting allowlist sizes..."
    ...
    # [18/29] docs/INVOCATION_TOPOLOGY_PLAN.md T1: a calibration nobody runs is a claim, not
    ...
    Write-Host "[18/29] Running every --calibrate self-test (list derived from argparse, not hand-written)..."
    ...
    # [19/29] O4 (Move 11 W2): the EXISTENCE half of the panel-provenance impact gate. Its only
    ...
    # printed PASS, same convention as [10/29].
    Write-Host "[19/29] Checking panel-provenance manifests against each built app's current model..."
    ...
    # [20/29] O4 (Move 11 W2): the emit-side mirror of [15/29]. Also invoked by npdev-pr-gate.yml
    ...
    Write-Host "[20/29] Checking every asserted DSL feature survives GENERATION, not just parsing..."
    ...
    # [21/29] O5 (Move 11 W4): does every step type declared in model.schema.json have a MODEL-level
    ...
    Write-Host "[21/29] Checking every step type has a model-level validation test..."
    ...
    # [22/29] Move 13 P6: makes docs/X0_SILENT_EXPRESSION_REGISTER.md's discipline a permanent gate
    ...
    Write-Host "[22/29] Checking the X0 silent-answer register against its machine-checked registry..."
    ...
    # [23/29] Fast Lane plan Sec.7.4 (2026-08-01): "a check absent from verification-cadence.json
    ...
    Write-Host "[23/29] Checking every run-all-gates.ps1 gate has a verification-cadence.json staleness deadline..."
    ...
    # [24/29] REG-110 (Move 14 Phase D): NPDevCli/tests/test_acceptance_runner.py -- the acceptance
    ...
    Write-Host "[24/29] Checking NPDevCli's own test suite (acceptance runner + dsl-2 migration)..."
    ...
    # [25/29] Move 14 Phase E item E1 (U2): "a rule applied in one place, not mirrored to its twin"
    ...
    Write-Host "[25/29] Checking registered twin-pair rules (REG-89/104/112's family) haven't diverged..."
    ...
    # [26/29] Move 15 Phase D item D1: five separate times a console/screen record said "blocked by
    ...
    Write-Host "[26/29] Checking cited blockers (REG-nn) in checklist/findings docs are still open..."
    ...
    # [27/29] S4 Phase B (S4_SPEC.md): does an ADR decision the owner accepted actually have live
    ...
    Write-Host "[27/29] Checking accepted ADR decisions carrying a decision-check claim are implemented..."
    ...
    # [28/29] storage/PLAN.md S1: the 41 dialect-bound SQL sites this repo used to carry inline were
    ...
    Write-Host "[28/29] Checking no dialect-bound SQL sits outside the dialect package..."
    ...
    # [29/29] docs/INVOCATION_TOPOLOGY_PLAN.md T2: every script under scripts/ must declare BOTH a
    ...
    Write-Host "[29/29] Checking every script declares a classification + invocation matching reality..."
