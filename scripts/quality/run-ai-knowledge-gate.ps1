...
try {
    ...
    # [1/44] Register self-check runs FIRST: platform-status.json is DERIVED from the same documents,
    ...
    Write-Host "[1/44] Checking register/roadmap summary rows against their detail sections..."
    ...
    # [5/44] Same question as [1/44], asked of a different mechanism: is this quality tool still doing
    ...
    Write-Host "[5/44] Checking the security pattern sweep still catches its known bugs..."
    ...
    # [6/44] Run the sweep against the CODEBASE and fail on anything untriaged. Until now the gate
    # proved the detector worked ([5/44]) but never actually pointed it at the repo, so a new hit could
    ...
    Write-Host "[6/44] Checking for untriaged security-pattern hits..."
    ...
    # [7/44] Report-only (Phase 2.4, docs/archive/programme-history/REMAINDER_CLOSURE_PLAN.md): calibrated against both real
    ...
    Write-Host "[7/44] Checking for narrative-status drift (report-only)..."
    & $py "scripts/quality/check-narrative-status-drift.py"

    # [8/44] T1.3: a custom Gradle Test task (behaviorTest/integrationTest/a future contractTest) that
    # is declared but reachable from no CI workflow is a test that only runs on one laptop -- exactly
    # what REG-49's residual behaviorTest was until T1.2 wired it in. Blocking, same rationale as [6/44].
    Write-Host "[8/44] Checking every custom Gradle Test task is reachable from a CI workflow..."
    ...
    # [9/44] 2.A.2 (docs/DSL2_AND_DECOMPOSITION_PLAN.md): model.schema.json is duplicated in four
    # places with nothing previously enforcing they stay in sync -- they had already drifted by the
    # time this gate was written. Blocking, same rationale as [6/44] and [8/44]: an unsynced schema
    ...
    Write-Host "[9/44] Checking the four model.schema.json copies are still semantically identical..."
    ...
    # [10/44] R-G1 static half (docs/REMEDIATION_PLAN.md): the panel-provenance impact gate (F4)
    ...
    Write-Host "[10/44] Checking *.panel.json manifests structurally validate (no live app needed)..."
    ...
    # [11/44] C4 (docs/CORPUS_INTEGRITY_PLAN.md): every model.json under AppGen/apps + NPDevSamples
    # must still parse against the REAL validator. Blocking, same rationale as [6/44]/[8/44]/[9/44] --
    ...
    Write-Host "[11/44] Checking every corpus model.json still parses..."
    ...
    Write-Host "[12/44] Checking every relative markdown link resolves..."
    ...
    # [13/44] Found by accident verifying F1/F2 on a live PR, 2026-07-29: an unquoted colon inside a
    ...
    Write-Host "[13/44] Checking every workflow file is valid YAML..."
    ...
    # [14/44] F6 (docs/FINAL_OPEN_ITEMS_PLAN.md): the simple-user-registry-* and p77-hookproof(-pg)
    ...
    Write-Host "[14/44] Checking engine-variant families stay byte-identical..."
    ...
    # [15/44] F8 (docs/FINAL_OPEN_ITEMS_PLAN.md): the corpus-parse gate (check 11) answers "does
    ...
    Write-Host "[15/44] Checking every DSL feature has at least one corpus model exercising it..."
    ...
    # [16/44] docs/RECORD_SURFACES_PLAN.md P4: two mechanical record-surface claims that go stale
    ...
    # rationale as [6/44]/[8/44]/[9/44]/[11/44]/[12/44]/[13/44].
    Write-Host "[16/44] Checking branch freshness vs. origin/main and CLAUDE.md's large-file size claims..."
    ...
    # [17/44] docs/FAIL_OPEN_PLAN.md R3: three allowlists (corpus-parse, test-task-coverage,
    ...
    Write-Host "[17/44] Checking allowlist entries carry a REG/B citation, reporting allowlist sizes..."
    ...
    # [18/44] docs/INVOCATION_TOPOLOGY_PLAN.md T1: a calibration nobody runs is a claim, not
    ...
    Write-Host "[18/44] Running every --calibrate self-test (list derived from argparse, not hand-written)..."
    ...
    # [19/44] O4 (Move 11 W2): the EXISTENCE half of the panel-provenance impact gate. Its only
    ...
    # printed PASS, same convention as [10/44].
    Write-Host "[19/44] Checking panel-provenance manifests against each built app's current model..."
    ...
    # [20/44] O4 (Move 11 W2): the emit-side mirror of [15/44]. Also invoked by npdev-pr-gate.yml
    ...
    Write-Host "[20/44] Checking every asserted DSL feature survives GENERATION, not just parsing..."
    ...
    # [21/44] O5 (Move 11 W4): does every step type declared in model.schema.json have a MODEL-level
    ...
    Write-Host "[21/44] Checking every step type has a model-level validation test..."
    ...
    # [22/44] Move 13 P6: makes docs/X0_SILENT_EXPRESSION_REGISTER.md's discipline a permanent gate
    ...
    Write-Host "[22/44] Checking the X0 silent-answer register against its machine-checked registry..."
    ...
    # [23/44] Fast Lane plan Sec.7.4 (2026-08-01): "a check absent from verification-cadence.json
    ...
    Write-Host "[23/44] Checking every run-all-gates.ps1 gate has a verification-cadence.json staleness deadline..."
    ...
    # [24/44] REG-110 (Move 14 Phase D): NPDevCli/tests/test_acceptance_runner.py -- the acceptance
    ...
    Write-Host "[24/44] Checking NPDevCli's own test suite (acceptance runner + dsl-2 migration)..."
    ...
    # [25/44] Move 14 Phase E item E1 (U2): "a rule applied in one place, not mirrored to its twin"
    ...
    Write-Host "[25/44] Checking registered twin-pair rules (REG-89/104/112's family) haven't diverged..."
    ...
    # [26/44] Move 15 Phase D item D1: five separate times a console/screen record said "blocked by
    ...
    Write-Host "[26/44] Checking cited blockers (REG-nn) in checklist/findings docs are still open..."
    ...
    # [27/44] S4 Phase B (S4_SPEC.md): does an ADR decision the owner accepted actually have live
    ...
    Write-Host "[27/44] Checking accepted ADR decisions carrying a decision-check claim are implemented..."
    ...
    # [28/44] storage/PLAN.md S1: the 41 dialect-bound SQL sites this repo used to carry inline were
    ...
    Write-Host "[28/44] Checking no dialect-bound SQL sits outside the dialect package..."
    ...
    # [29/44] storage/FULL_SUPPORT_PLAN.md W1.1: the storage conformance suite must run on DIGESTS,
    ...
    Write-Host "[29/44] Checking conformance container images are pinned to digests..."
    ...
    # [30/44] storage/FULL_SUPPORT_PLAN.md W3, and the mechanical half of STOR-2. A storage message
    ...
    Write-Host "[30/44] Checking no storage message claims a rollback without asking the dialect..."
    ...
    # [31/44] storage/FULL_SUPPORT_PLAN.md W6.1. Every config.json carries a $schema pointer to
    ...
    Write-Host "[31/44] Validating every corpus config.json against config.schema.json..."
    ...
    # [32/44] storage/OPEN_ITEMS_PLAN.md W7 (ledger STOR-5): the guarded-DDL idioms NPDev emits must
    # be ones the target engine can actually run. check-dialect-sites.py [29/44] guards the SOURCE;
    ...
    Write-Host "[32/44] Checking emitted SQL is portable to the engine each script was generated for..."
    ...
    # clean today.
    Write-Host "[33/44] Checking every engine gets the same environment toolbox..."
    ...
    Write-Host "[34/44] Checking no template resource shadows a generated one..."
    ...
    # [35/44] The mirror of [28/44]: that one fails when SQL is written OUTSIDE the dialect package,
    ...
    Write-Host "[35/44] Checking every SqlDialect answer has a caller that asks it..."
    ...
    # [36/44] PRE_ROUND_FIXES.md section 4: the STATIC half of what the first-run harness proves.
    ...
    Write-Host "[36/44] Checking every documented command/default a reader would copy..."
    ...
    # [37/44] QUAL-2 / round3 Part 2.1: a Files.list|walk|find|lines|newDirectoryStream whose Stream
    ...
    # very line.
    Write-Host "[37/44] Checking every Files stream is closed (try-with-resources)..."
    ...
    # [38/44] FOUR_AND_EXTERNAL.md A.1: the first-run harness's own command extractor, against a
    ...
    Write-Host "[38/44] Checking the first-run harness's command extractor against its corpus..."
    ...
    # [39/44] Any file declaring a shebang must have LF line endings. `core.autocrlf=true` is Git
    ...
    Write-Host "[39/44] Checking every shebang'd file has LF line endings..."
    ...
    # [40/44] The REVERSE of [26/44], and the direction nobody was testing. That check catches a doc
    ...
    # and [37/44] independently proved the family gone across 1464 Java files. That matters because
    ...
    # four real items. Blocking, same rationale as [26/44].
    Write-Host "[40/44] Checking no OPEN ledger item has a fix that already landed..."
    ...
    # [41/44] docs/INVOCATION_TOPOLOGY_PLAN.md T2: every script under scripts/ must declare BOTH a
    ...
    # [41/44] A download link in an install guide must not name a version. Found twice on 2026-08-10,
    ...
    Write-Host "[41/44] Checking no install guide pins a specific release in a download link..."
    ...
    # [42/44] PORT-1's CLASS, which fixing its instance did not close. The committed React bundle
    ...
    Write-Host "[42/44] Checking committed generated artefacts are not older than their source..."
    ...
    # [43/44] MONITOR_PLAN A3. The routine vocabulary is the ENGINE's, pinned into
    ...
    # looked at. Self-tests first, for the same reason check-engine-parity.py does [33/44]: a
    # checker that can no longer tell a broken routine from a good one has a meaningless verdict.
    Write-Host "[43/44] Checking every checked-in browser routine conforms to the pinned engine schema..."
    ...
    Write-Host "[44/44] Checking every script declares a classification + invocation matching reality..."
