...
.DESCRIPTION
    ...
    check-blocker-citation-freshness.py (see [36/34] below). All 177 items the first guarded were
...
try {
    ...
    if ($Fix) {
        Write-Host "[1/34] Regenerating gaps roadmap + platform-status projection..." -ForegroundColor Yellow
        ...
        Write-Host "[1/34] Checking gaps roadmap generates + platform-status projection is current..."
    ...
    Write-Host "[2/34] Validating knowledge cards..."
    & $py "scripts/ai/build_knowledge.py" --validate-only
    if ($LASTEXITCODE -ne 0) { $failures += "knowledge-card validation failed" }

    # Bundled into the same [1/34] slot (docs-decoupling-2026-08-11 PLAN.md Phase 1's precedent:
    ...
    Write-Host "[3/34] Checking failure-signature normalizer..."
    ...
    # [4/34] The same question check-register-consistency.py (deleted, md-zero-2026-08-11 PLAN.md
    ...
    Write-Host "[4/34] Checking the security pattern sweep still catches its known bugs..."
    ...
    # [5/34] Run the sweep against the CODEBASE and fail on anything untriaged. Until now the gate
    # proved the detector worked ([3/34]) but never actually pointed it at the repo, so a new hit could
    ...
    Write-Host "[5/34] Checking for untriaged security-pattern hits..."
    ...
    # [6/34] T1.3: a custom Gradle Test task (behaviorTest/integrationTest/a future contractTest) that
    # is declared but reachable from no CI workflow is a test that only runs on one laptop -- exactly
    # what REG-49's residual behaviorTest was until T1.2 wired it in. Blocking, same rationale as [4/34].
    Write-Host "[6/34] Checking every custom Gradle Test task is reachable from a CI workflow..."
    ...
    # [7/34] 2.A.2 (docs/DSL2_AND_DECOMPOSITION_PLAN.md): model.schema.json is duplicated in four
    # places with nothing previously enforcing they stay in sync -- they had already drifted by the
    # time this gate was written. Blocking, same rationale as [4/34] and [5/34]: an unsynced schema
    ...
    Write-Host "[7/34] Checking the four model.schema.json copies are still semantically identical..."
    ...
    # [8/34] R-G1 static half (docs/REMEDIATION_PLAN.md): the panel-provenance impact gate (F4)
    ...
    Write-Host "[8/34] Checking *.panel.json manifests structurally validate (no live app needed)..."
    ...
    # [9/34] C4 (docs/CORPUS_INTEGRITY_PLAN.md): every model.json under AppGen/apps + NPDevSamples
    # must still parse against the REAL validator. Blocking, same rationale as [4/34]/[5/34]/[6/34] --
    ...
    Write-Host "[9/34] Checking every corpus model.json still parses..."
    ...
    # [10/34] Found by accident verifying F1/F2 on a live PR, 2026-07-29: an unquoted colon inside a
    ...
    Write-Host "[10/34] Checking every workflow file is valid YAML..."
    ...
    # [11/34] F6 (docs/FINAL_OPEN_ITEMS_PLAN.md): the simple-user-registry-* and p77-hookproof(-pg)
    ...
    Write-Host "[11/34] Checking engine-variant families stay byte-identical..."
    ...
    # [12/34] F8 (docs/FINAL_OPEN_ITEMS_PLAN.md): the corpus-parse gate (check 11) answers "does
    ...
    Write-Host "[12/34] Checking every DSL feature has at least one corpus model exercising it..."
    ...
    # [13/34] docs/INVOCATION_TOPOLOGY_PLAN.md T1: a calibration nobody runs is a claim, not
    ...
    Write-Host "[13/34] Running every --calibrate self-test (list derived from argparse, not hand-written)..."
    ...
    # [14/34] O4 (Move 11 W2): the EXISTENCE half of the panel-provenance impact gate. Its only
    ...
    # printed PASS, same convention as [7/34].
    Write-Host "[14/34] Checking panel-provenance manifests against each built app's current model..."
    ...
    # [15/34] O4 (Move 11 W2): the emit-side mirror of [12/34]. Also invoked by npdev-pr-gate.yml
    ...
    Write-Host "[15/34] Checking every asserted DSL feature survives GENERATION, not just parsing..."
    ...
    # [16/34] O5 (Move 11 W4): does every step type declared in model.schema.json have a MODEL-level
    ...
    Write-Host "[16/34] Checking every step type has a model-level validation test..."
    ...
    # [17/34] Move 13 P6: makes the X0 silent-answer discipline a permanent gate instead of something
    ...
    Write-Host "[17/34] Checking the X0 silent-answer registry..."
    ...
    # [18/34] REG-110 (Move 14 Phase D): NPDevCli/tests/test_acceptance_runner.py -- the acceptance
    ...
    Write-Host "[18/34] Checking NPDevCli's own test suite (acceptance runner + dsl-2 migration)..."
    ...
    # [19/34] Move 14 Phase E item E1 (U2): "a rule applied in one place, not mirrored to its twin"
    ...
    Write-Host "[19/34] Checking registered twin-pair rules (REG-89/104/112's family) haven't diverged..."
    ...
    # [20/34] storage/PLAN.md S1: the 41 dialect-bound SQL sites this repo used to carry inline were
    ...
    Write-Host "[20/34] Checking no dialect-bound SQL sits outside the dialect package..."
    ...
    # [21/34] storage/FULL_SUPPORT_PLAN.md W1.1: the storage conformance suite must run on DIGESTS,
    ...
    Write-Host "[21/34] Checking conformance container images are pinned to digests..."
    ...
    # [22/34] storage/FULL_SUPPORT_PLAN.md W3, and the mechanical half of STOR-2. A storage message
    ...
    Write-Host "[22/34] Checking no storage message claims a rollback without asking the dialect..."
    ...
    # [23/34] storage/FULL_SUPPORT_PLAN.md W6.1. Every config.json carries a $schema pointer to
    ...
    Write-Host "[23/34] Validating every corpus config.json against config.schema.json..."
    ...
    # [24/34] storage/OPEN_ITEMS_PLAN.md W7 (ledger STOR-5): the guarded-DDL idioms NPDev emits must
    # be ones the target engine can actually run. check-dialect-sites.py [25/34] guards the SOURCE;
    ...
    Write-Host "[24/34] Checking emitted SQL is portable to the engine each script was generated for..."
    ...
    # clean today.
    Write-Host "[25/34] Checking every engine gets the same environment toolbox..."
    ...
    Write-Host "[26/34] Checking no template resource shadows a generated one..."
    ...
    # [27/34] The mirror of [24/34]: that one fails when SQL is written OUTSIDE the dialect package,
    ...
    Write-Host "[27/34] Checking every SqlDialect answer has a caller that asks it..."
    ...
    # [28/34] QUAL-2 / round3 Part 2.1: a Files.list|walk|find|lines|newDirectoryStream whose Stream
    ...
    # very line.
    Write-Host "[28/34] Checking every Files stream is closed (try-with-resources)..."
    ...
    # [29/34] FOUR_AND_EXTERNAL.md A.1: the first-run harness's own command extractor, against a
    ...
    Write-Host "[29/34] Checking the first-run harness's command extractor against its corpus..."
    ...
    # [30/34] Any file declaring a shebang must have LF line endings. `core.autocrlf=true` is Git
    ...
    Write-Host "[30/34] Checking every shebang'd file has LF line endings..."
    ...
    # [31/34] PORT-1's CLASS, which fixing its instance did not close. The committed React bundle
    ...
    Write-Host "[31/34] Checking committed generated artefacts are not older than their source..."
    ...
    # [32/34] MONITOR_PLAN A3. The routine vocabulary is the ENGINE's, pinned into
    ...
    # looked at. Self-tests first, for the same reason check-engine-parity.py does [29/34]: a
    # checker that can no longer tell a broken routine from a good one has a meaningless verdict.
    Write-Host "[32/34] Checking every checked-in browser routine conforms to the pinned engine schema..."
    ...
    # [33/34] docs/INVOCATION_TOPOLOGY_PLAN.md T2: every script under scripts/ must declare BOTH a
    ...
    Write-Host "[33/34] Checking every script declares a classification + invocation matching reality..."
    ...
    # acceptable latency for a hygiene rule (unlike, say, [3/34]/[5/34]/[6/34]'s bug classes, which
    # stay in the per-commit gate).

    # [41/34] R3 (MASTER-ROADMAP.md Step 9 / ledger QUAL-6): the coverage ratchet. Static by
    ...
    Write-Host "[34/34] Checking JaCoCo coverage hasn't dropped below its recorded floor..."
