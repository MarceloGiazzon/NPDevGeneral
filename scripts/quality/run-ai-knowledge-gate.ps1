...
try {
    ...
    # [1/31] Register self-check runs FIRST: platform-status.json is DERIVED from the same documents,
    ...
    Write-Host "[1/31] Checking register/roadmap summary rows against their detail sections..."
    ...
    if ($Fix) {
        Write-Host "[2/31] Regenerating platform-status projection..." -ForegroundColor Yellow
        & $py "scripts/ai/extract_platform_status.py"
        if ($LASTEXITCODE -ne 0) { $failures += "platform-status regeneration failed" }
    } else {
        Write-Host "[2/31] Checking platform-status projection is current..."
    ...
    # [5/31] Same question as [1/31], asked of a different mechanism: is this quality tool still doing
    ...
    Write-Host "[5/31] Checking the security pattern sweep still catches its known bugs..."
    ...
    # [6/31] Run the sweep against the CODEBASE and fail on anything untriaged. Until now the gate
    # proved the detector worked ([5/31]) but never actually pointed it at the repo, so a new hit could
    ...
    Write-Host "[6/31] Checking for untriaged security-pattern hits..."
    ...
    # [7/31] Report-only (Phase 2.4, docs/archive/programme-history/REMAINDER_CLOSURE_PLAN.md): calibrated against both real
    ...
    Write-Host "[7/31] Checking for narrative-status drift (report-only)..."
    & $py "scripts/quality/check-narrative-status-drift.py"

    # [8/31] T1.3: a custom Gradle Test task (behaviorTest/integrationTest/a future contractTest) that
    # is declared but reachable from no CI workflow is a test that only runs on one laptop -- exactly
    # what REG-49's residual behaviorTest was until T1.2 wired it in. Blocking, same rationale as [6/31].
    Write-Host "[8/31] Checking every custom Gradle Test task is reachable from a CI workflow..."
    ...
    # [9/31] 2.A.2 (docs/DSL2_AND_DECOMPOSITION_PLAN.md): model.schema.json is duplicated in four
    # places with nothing previously enforcing they stay in sync -- they had already drifted by the
    # time this gate was written. Blocking, same rationale as [6/31] and [8/31]: an unsynced schema
    ...
    Write-Host "[9/31] Checking the four model.schema.json copies are still semantically identical..."
    ...
    # [10/31] R-G1 static half (docs/REMEDIATION_PLAN.md): the panel-provenance impact gate (F4)
    ...
    Write-Host "[10/31] Checking *.panel.json manifests structurally validate (no live app needed)..."
    ...
    # [11/31] C4 (docs/CORPUS_INTEGRITY_PLAN.md): every model.json under AppGen/apps + NPDevSamples
    # must still parse against the REAL validator. Blocking, same rationale as [6/31]/[8/31]/[9/31] --
    ...
    Write-Host "[11/31] Checking every corpus model.json still parses..."
    ...
    Write-Host "[12/31] Checking every relative markdown link resolves..."
    ...
    # [13/31] Found by accident verifying F1/F2 on a live PR, 2026-07-29: an unquoted colon inside a
    ...
    Write-Host "[13/31] Checking every workflow file is valid YAML..."
    ...
    # [14/31] F6 (docs/FINAL_OPEN_ITEMS_PLAN.md): the simple-user-registry-* and p77-hookproof(-pg)
    ...
    Write-Host "[14/31] Checking engine-variant families stay byte-identical..."
    ...
    # [15/31] F8 (docs/FINAL_OPEN_ITEMS_PLAN.md): the corpus-parse gate (check 11) answers "does
    ...
    Write-Host "[15/31] Checking every DSL feature has at least one corpus model exercising it..."
    ...
    # [16/31] docs/RECORD_SURFACES_PLAN.md P4: two mechanical record-surface claims that go stale
    ...
    # rationale as [6/31]/[8/31]/[9/31]/[11/31]/[12/31]/[13/31].
    Write-Host "[16/31] Checking branch freshness vs. origin/main and CLAUDE.md's large-file size claims..."
    ...
    # [17/31] docs/FAIL_OPEN_PLAN.md R3: three allowlists (corpus-parse, test-task-coverage,
    ...
    Write-Host "[17/31] Checking allowlist entries carry a REG/B citation, reporting allowlist sizes..."
    ...
    # [18/31] docs/INVOCATION_TOPOLOGY_PLAN.md T1: a calibration nobody runs is a claim, not
    ...
    Write-Host "[18/31] Running every --calibrate self-test (list derived from argparse, not hand-written)..."
    ...
    # [19/31] O4 (Move 11 W2): the EXISTENCE half of the panel-provenance impact gate. Its only
    ...
    # printed PASS, same convention as [10/31].
    Write-Host "[19/31] Checking panel-provenance manifests against each built app's current model..."
    ...
    # [20/31] O4 (Move 11 W2): the emit-side mirror of [15/31]. Also invoked by npdev-pr-gate.yml
    ...
    Write-Host "[20/31] Checking every asserted DSL feature survives GENERATION, not just parsing..."
    ...
    # [21/31] O5 (Move 11 W4): does every step type declared in model.schema.json have a MODEL-level
    ...
    Write-Host "[21/31] Checking every step type has a model-level validation test..."
    ...
    # [22/31] Move 13 P6: makes docs/X0_SILENT_EXPRESSION_REGISTER.md's discipline a permanent gate
    ...
    Write-Host "[22/31] Checking the X0 silent-answer register against its machine-checked registry..."
    ...
    # [23/31] Fast Lane plan Sec.7.4 (2026-08-01): "a check absent from verification-cadence.json
    ...
    Write-Host "[23/31] Checking every run-all-gates.ps1 gate has a verification-cadence.json staleness deadline..."
    ...
    # [24/31] REG-110 (Move 14 Phase D): NPDevCli/tests/test_acceptance_runner.py -- the acceptance
    ...
    Write-Host "[24/31] Checking NPDevCli's own test suite (acceptance runner + dsl-2 migration)..."
    ...
    # [25/31] Move 14 Phase E item E1 (U2): "a rule applied in one place, not mirrored to its twin"
    ...
    Write-Host "[25/31] Checking registered twin-pair rules (REG-89/104/112's family) haven't diverged..."
    ...
    # [26/31] Move 15 Phase D item D1: five separate times a console/screen record said "blocked by
    ...
    Write-Host "[26/31] Checking cited blockers (REG-nn) in checklist/findings docs are still open..."
    ...
    # [27/31] S4 Phase B (S4_SPEC.md): does an ADR decision the owner accepted actually have live
    ...
    Write-Host "[27/31] Checking accepted ADR decisions carrying a decision-check claim are implemented..."
    ...
    # [28/31] storage/PLAN.md S1: the 41 dialect-bound SQL sites this repo used to carry inline were
    ...
    Write-Host "[28/31] Checking no dialect-bound SQL sits outside the dialect package..."
    ...
    # [29/31] storage/FULL_SUPPORT_PLAN.md W1.1: the storage conformance suite must run on DIGESTS,
    ...
    Write-Host "[29/31] Checking conformance container images are pinned to digests..."
    ...
    # [30/31] storage/FULL_SUPPORT_PLAN.md W3, and the mechanical half of STOR-2. A storage message
    ...
    Write-Host "[30/31] Checking no storage message claims a rollback without asking the dialect..."
    ...
    # [31/31] docs/INVOCATION_TOPOLOGY_PLAN.md T2: every script under scripts/ must declare BOTH a
    ...
    Write-Host "[31/31] Checking every script declares a classification + invocation matching reality..."
