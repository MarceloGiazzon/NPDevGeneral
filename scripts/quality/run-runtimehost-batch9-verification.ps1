...
$scriptPath = 'scripts\quality\run-runtimehost-batch9-verification.ps1'
$outPath = Join-Path $WorkspaceRoot 'scripts\reports\out\runtimehost-batch9-verification-report.json'
$report = New-StructuredRunReport -WorkspaceRoot $WorkspaceRoot -ScriptPath $scriptPath -RunIdPrefix 'runtimehost-batch9-verification'
...
if ($report.overallStatus -eq 'failed') {
    Write-Warning 'RuntimeHost Batch 9 verification failed.'
    throw 'RuntimeHost Batch 9 verification failed.'
}

Write-Host 'OK    RuntimeHost Batch 9 verification completed.'
