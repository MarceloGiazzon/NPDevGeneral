...
$scriptPath = 'scripts\quality\run-runtimehost-batch11-verification.ps1'
$outPath = Join-Path $WorkspaceRoot 'scripts\reports\out\runtimehost-batch11-verification-report.json'
$report = New-StructuredRunReport -WorkspaceRoot $WorkspaceRoot -ScriptPath $scriptPath -RunIdPrefix 'runtimehost-batch11-verification'
...
if ($report.overallStatus -eq 'failed') {
    Write-Warning 'RuntimeHost Batch 11 verification failed.'
    throw 'RuntimeHost Batch 11 verification failed.'
}

Write-Host 'OK    RuntimeHost Batch 11 verification completed.'
