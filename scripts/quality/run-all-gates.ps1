...
$gates = @(
    [pscustomobject]@{ Name = "aiKnowledge"; Script = "scripts/quality/run-ai-knowledge-gate.ps1"; Why = "static instrument checks + all 13 check-*.py" }
    [pscustomobject]@{ Name = "generator";   Script = "scripts/quality/run-generator-gate.ps1";    Why = "codegen engine" }
    [pscustomobject]@{ Name = "runtimeHost"; Script = "scripts/quality/run-runtimehost-gate.ps1";  Why = "assembled sample app + its test suite" }
    [pscustomobject]@{ Name = "frontend";    Script = "scripts/quality/run-frontend-gate.ps1";     Why = "authoring UI" }
    [pscustomobject]@{ Name = "betaRelease"; Script = "scripts/quality/run-beta-release-gate.ps1"; Why = "aggregate release checks" }
)
