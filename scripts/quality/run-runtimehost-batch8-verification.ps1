...
$scriptPath = 'scripts\quality\run-runtimehost-batch8-verification.ps1'
$outPath = Join-Path $WorkspaceRoot 'scripts\reports\out\runtimehost-batch8-verification-report.json'
$report = New-StructuredRunReport -WorkspaceRoot $WorkspaceRoot -ScriptPath $scriptPath -RunIdPrefix 'runtimehost-batch8-verification'
...
if ($report.overallStatus -eq 'failed') {
    Write-Warning 'RuntimeHost Batch 8 verification failed.'
    throw 'RuntimeHost Batch 8 verification failed.'
}

Write-Host 'OK    RuntimeHost Batch 8 verification completed.'
