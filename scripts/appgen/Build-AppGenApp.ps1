...
.DESCRIPTION
  ...
  -App accepts EITHER a short app name (resolved under -AppsRoot) OR a full path to the
  app folder (the folder that contains a 'definition' subfolder).

.EXAMPLE
  .\Build-AppGenApp.ps1 -App Pigmentampa
  .\Build-AppGenApp.ps1 -App 'D:\WorkSpace\NPDev\AppGen\apps\Pigmentampa'
...
if (Test-Path -LiteralPath (Join-Path $App 'definition')) {
  ...
  $appFolder = Join-Path $AppsRoot $App
}
if (-not (Test-Path -LiteralPath (Join-Path $appFolder 'definition'))) {
  $available = @(Get-ChildItem -LiteralPath $AppsRoot -Directory -ErrorAction SilentlyContinue |
