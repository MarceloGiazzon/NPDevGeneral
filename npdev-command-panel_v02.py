...
MAX_OUT = 200_000  # Cap backend log memory to ~200 KB per run
        ...
        for line in proc.stdout:
            with RUNS_LOCK:
                ...
                if run is None:
                    return
                run["out"] += line
                if len(run["out"]) > MAX_OUT:
                    # Truncate to tail end to bound memory usage
                    run["out"] = "[... output truncated ...]\n" + run["out"][-MAX_OUT:]
        proc.wait()
    finally:
        with RUNS_LOCK:
...
<style>
  ...
  * { box-sizing:border-box; }
  ...
  .outtop { padding:10px 16px; border-bottom:1px solid var(--border); display:flex; align-items:center; gap:12px; background:#fff; flex-wrap:wrap; }
  .outtop .title { font-size:13px; font-weight:600; flex:1; }
  .limit-control { display:flex; align-items:center; gap:6px; font-size:12px; color:#64748b; }
  .limit-control select { font-size:12px; border:1px solid var(--border); border-radius:4px; padding:3px 6px; }
  ...
  <div class="outwrap">
    <div class="outtop">
      ...
      <div class="limit-control">
        <label for="max-chars">Max Buffer:</label>
        <select id="max-chars">
          <option value="20000">20k chars (~400 lines)</option>
          <option value="50000">50k chars (~1000 lines)</option>
          <option value="100000" selected>100k chars (~2000 lines)</option>
          <option value="200000">200k chars (~4000 lines)</option>
          <option value="500000">500k chars (~10000 lines)</option>
        </select>
      </div>
...
const maxCharsSelect = document.getElementById('max-chars');
let pollTimer = null;
let currentId = null;
...
function updateOutput(text) {
  const limit = parseInt(maxCharsSelect.value, 10);
  let displayText = text;
  
  if (displayText.length > limit) {
    displayText = '... [Output capped to prevent unresponsiveness] ...' + displayText.slice(-limit);
  }

  // Use textContent directly to avoid huge DOM trees
  outEl.textContent = displayText;
...
async function run(key, label) {
  ...
  outEl.textContent = 'Starting process...';
  ...
  try {
    ...
    outEl.textContent = 'ERROR: ' + e.message; currentId = null; killBtn.style.display='none'; return;
  }
  currentId = id;
...
function killCurrent(silent) {
  ...
  if (!silent) { setStatus('fail', 'killed'); }
  currentId = null;
}
...
copyBtn.addEventListener('click', async () => {
  ...
  if (!text || text.startsWith('Pick a command')) return;
  let ok = false;
  try {
...
document.getElementById('clear').addEventListener('click', () => { 
  outEl.innerHTML = '<span class="empty">Pick a command on the left. Output appears here.</span>'; 
});
</script>
</body>
</html>
