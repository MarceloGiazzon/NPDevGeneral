...
type BehaviorWriteBackResponse = {
  requestId: string;
  requestType: string;
  status: string;
  submittedAt: string;
...
export type SemanticBehaviorWriteBackPanelProps = {
  catalog: BehaviorWriteBackCatalog;
  apiKey?: string;
};

const SEMANTIC_BEHAVIOR_WRITEBACK_ENDPOINT = "/api/admin/model/semantic-behavior-writeback";

export default function SemanticBehaviorWriteBackPanel(
  props: SemanticBehaviorWriteBackPanelProps
) {
  ...
  async function loadHistory() {
    const response = await fetch(`${SEMANTIC_BEHAVIOR_WRITEBACK_ENDPOINT}/history`, {
      headers: {
        "X-API-Key": apiKey
      }
    });
  ...
  async function submitRequest(request: Record<string, unknown>) {
    ...
    try {
      ...
      const response = await fetch(SEMANTIC_BEHAVIOR_WRITEBACK_ENDPOINT, {
        method: "POST",
        headers: {
      ...
      const parsed: BehaviorWriteBackResponse = JSON.parse(text);
  ...
  return (
    <div style={{ padding: "16px" }}>
      ...
      <p>
        Submit rule, lifecycle, and orchestration write-back requests to the runtime-backed API.
      </p>

      <label htmlFor="behavior-writeback-type">
        <strong>Request type</strong>
      </label>
      <div style={{ margin: "8px 0 16px 0" }}>
        <select
          id="behavior-writeback-type"
          value={selectedType}
