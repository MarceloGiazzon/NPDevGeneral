 ...
 * {@code EMIT_EVENT} step-kind case body, split out of {@link KernelRunner#executeSteps}'s switch
 * verbatim -- no behavior change. See {@link InvariantCheckStep} for the shared {@code execute}
 * return convention. Deliberately near-identical to {@link ScheduleEventStep} (the pre-existing
 * duplication between the two step kinds is preserved as-is, not deduplicated, per this split's
 * pure-move discipline). Stays a flat sibling of {@link KernelRunner} in {@code com.npdev.kernel},
 * not a subpackage, for the same reason the rest of this split's files do: the collaborators
 * (eventSchemaProvider, schemaValidator, eventStore, eventBus, nextId, traceFailedStep) are
 * package-private.
 */
final class EmitEventStep {

    private EmitEventStep() {
    }
