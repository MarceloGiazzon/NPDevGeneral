...
public final class PostgresDialect implements SqlDialect {
    ...
    private String rowLimitSuffix(long rows) {
        if (rows <= 0) {
            throw new IllegalArgumentException("engine 'postgres': rowLimited must be positive, got " + rows);
        }
    ...
    public String guardedDropColumn(String tableName, String columnName, String alterStatement) {
        // Postgres supports DROP COLUMN IF EXISTS natively (STOR-35/P7) -- the plain statement, guarded.
