...
| Item | State | Notes |
|---|---|---|
| **REG-6 — ColumnFacts** | **CLOSED as re-scoped 2026-07-22** | Risk-core done (projection + drift-guard + semantic-pass migrations, 2026-07-21) and a class-header directive now requires any NEW pass to read `ColumnFacts`. Full set-algebra purity formally DEFERRED per the register's own rationale. Reopen trigger: adding a new executor pass. See register §1.6. |
...
| **Latent item — wmsoffice `D:/` JWT paths** | **CLOSED 2026-07-22 (verified + dispositioned)** | Real (`application-wmsoffice.yml` → `D:/WorkSpace/NPDev/Build/wmsoffice-keys/jwt-{public,private-pkcs8}.pem`), but it is layer-2 local-deployment config for a locally-run app (documented in the app's own README), and the platform's portable path exists: `NPDEV_AUTH_JWT_PUBLICKEYPATH`/`PRIVATEKEYPATH` env-var overrides (REG-9, emitted into compose). Disposition: sanctioned local convention, not a platform defect. |
| **Latent item — adapter-list fragility** | **CLOSED 2026-07-22 (guarded)** | `AdapterRegistrationConsistencyTest` (generator test module) now asserts every `NPDevKernel/settings.gradle` adapter appears in ALL 3 packaged-app proof-test lists (or in an explicit `KNOWN_NOT_PACKAGED` set) and that the 3 lists stay identical. RED-proven: it caught a live drift on arrival (`TrustedSource…` was missing `file-store-objectstore`; fixed). Sync script verified list-free. Checklist: `docs/ADAPTER_REGISTRATION_CHECKLIST.md`. |
| **Latent item — platform-status drift** | **CLOSED 2026-07-22 (was already guarded)** | The "no automation guards staleness" claim was stale: `run-ai-knowledge-gate.ps1` regenerates AND `--check`s `knowledge/platform-status.json`, and `.github/workflows/ai-knowledge-gate.yml` runs that gate in CI. Re-verified passing locally 2026-07-22. Only this ledger correction was needed. |
| **REG-17 (third-party reproduction)** | **ADVANCED 2026-07-22 — findings filed** | Full Linux maturity validation executed on GitHub ubuntu (run 29974176793) from clean `main`; failures recorded in `NPDev_General__OutsideRepo/reg17-linux-validation-2026-07-22/`. Mechanism proven external; green re-run pending the filed fixes. |
