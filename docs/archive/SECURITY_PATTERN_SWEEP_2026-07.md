...
### 4.11 → 2026-08-14, R8c/RUN-2 (`SqlDialect.selectForUpdateSkipLocked`, the flow-resume claim)

| Lead | Hits | Resolution |
|---|---|---|
| `H2Dialect.java:138` / `MySqlDialect.java:183` / `PostgresDialect.java:132` — the shared `SELECT ... WHERE ... ORDER BY ... LIMIT ... FOR UPDATE SKIP LOCKED` shape | 6 (3 files × 2 rules) | Rules **read-without-tenant-predicate** + **sql-string-building**, cleared safe (fingerprints `a7f96d98b862` / `e8e107c7cb4a`). New method, same architecture as `selectForUpdate` (§4.6-adjacent, STOR-9, fingerprint `95ea619c7299`/`399bd24efbec`/etc.): the dialect is handed `columns`, `table`, the WHOLE `whereClause` and `orderBy` by its ONE caller (`JdbcFlowInstanceStore.claimWaitingEligibleToResume`, R8c's flow-resume claim) and only decides WHERE the row lock goes and how the row cap/SKIP LOCKED is spelled — it adds no predicate and removes none. Unlike `selectForUpdate`'s two tenant-independent-by-design callers, this one genuinely IS tenant-scoped by its caller: the passed `whereClause` literally begins `tenant_id = ? AND status = 'WAITING_EVENT' AND ...`, bound via `PreparedStatement`. `columns`/`table`/`orderBy` are hardcoded Java literals at that one call site; `maxRows` is an int clamped to `[1,1000]` by `normalizeLimit` before the call, never a caller-supplied string. Not model-author-reachable. |
| `SqlServerDialect.java:184` — the `SELECT TOP (?) ... WITH (UPDLOCK, ROWLOCK, READPAST) WHERE ... ORDER BY ...` shape | 2 (1 file × 2 rules) | Rules **read-without-tenant-predicate** + **sql-string-building**, cleared safe (fingerprints `49603d5fe9a0` / `4299147bde24`). Same method, same caller, same reasoning as the row above — this shape differs only in HOW SQL Server spells the same two things every other engine's shape also spells: `TOP (maxRows)` is a prefix of the select list (T-SQL has no suffix row cap) and `WITH (UPDLOCK, ROWLOCK, READPAST)` is a table hint before the WHERE (T-SQL has no `FOR UPDATE` outside a cursor and no `SKIP LOCKED` keyword; `READPAST` is the documented equivalent). |

### 4.12 → 2026-08-14, REG-160 (`WorkspaceMenuSeeder` dynamic table-name resolution)

| Lead | Hits | Resolution |
|---|---|---|
| `WorkspaceMenuSeeder.java:168,191,205,216,239` — `countExistingMenus`/`findExistingFingerprint`/`deleteExistingMenus`/`insertFingerprintRow`/`insertMenuRow`'s SQL | 5 | Rule **sql-string-building**, cleared safe (fingerprints `e77af6717a3b` / `a48f5f23e7b5` / `2d3341e8a109` / `3e12ca10afd7` / `2d8edc262871`). REG-160 fixed a hardcoded, pre-versioning literal (`workspace_menus` instead of the generator's actual `workspace_v1_menus`) by resolving the table name dynamically instead: a new `menuTable` field, set ONCE in the constructor via `resolveMenuTable(compiledModel) -> compiledModel.findConcept("workspace::Menu").map(CompiledConcept::getTableName)` — the exact same compiled-model source of truth the generator itself used to CREATE the table, never a caller-, request-, or runtime-supplied value. Same posture as `JdbcBusinessConceptStore`'s own compiled-model-derived table splicing (§2, fingerprint `3cce05597d93`): every actual VALUE (`tenant_id`, `label`, `target`, `kind`, `ordinal`, ...) is bound via `PreparedStatement.setString`/`setObject`; only the identifier is spliced, which is unavoidable since JDBC bind parameters cannot parameterize identifiers. New fingerprints because this replaces the previous hardcoded-literal statements. |

### 4.13 → 2026-08-15, REG-170 (`IdentityProvisioning`/`ControlPanelTenantUsersController` dynamic identity table-name resolution)

| Lead | Hits | Resolution |
|---|---|---|
| `IdentityProvisioning.java:64,76,101,156,168,181` — `countUsersInTenant`/`usernameTaken`/`insertIdentityUser`/`findOrCreateRole`/`insertUserRole`'s SQL | 6 | Rule **sql-string-building**, cleared safe (fingerprints `39c424e2155a` / `cb6e6b5811c6` / `cb55326e4958` / `dff0e2a900b1` / `03eef6937f1d` / `532aed4a872c`). Same shape and same resolution as §4.12's REG-160 precedent: REG-170 fixed hardcoded, pre-versioning literals (`identity_users`/`identity_roles`/`identity_user_roles`) by resolving all four identity-pack table names ONCE per caller via a new `IdentityTableNames` record (`IdentityTableNames.resolve(compiledModel) -> compiledModel.findConcept("identity::User"/"identity::Role"/"identity::UserRole"/"identity::UserRolePermission").map(CompiledConcept::getTableName)`) — the same compiled-model source of truth the generator itself used to CREATE these tables, never a caller-, request-, or runtime-supplied value. Same posture as `JdbcBusinessConceptStore`'s own compiled-model-derived table splicing (§2, fingerprint `3cce05597d93`): every actual VALUE is bound via `PreparedStatement.setString`/`setObject`/`setObject`; only the identifier is spliced. New fingerprints because this replaces the previous hardcoded-literal statements. |
| `ControlPanelTenantUsersController.java:295,296,341,404,449,491,510,633` — `revokeRole`/`listPermissionOverrides`/`grantPermissionOverride`/`revokePermissionOverride`/`overrideRowExists`/`findUserRoleId`/`rolesOf`'s own raw SQL (independent of `IdentityProvisioning`, same class of literal) | 8 | Rule **sql-string-building**, cleared safe (fingerprints `ae247fe88c3f` / `84a79e60ead0` / `0ec10f1d25b1` / `53f36ccfd14d` / `a59f47f99d3b` / `bc67fcd93e10` / `78fe5ae366a7` / `af4f1ede4b70`). Same resolution and same `IdentityTableNames` field, resolved once in this controller's own constructor from its already-injected `CompiledModel` bean. `findUserRoleId`/`rolesOf` additionally splice the controller's own pre-existing, separately-configurable `userTable`/`usernameColumn` fields (`@Value("${npdev.auth.login.user-table:identity_users}")` etc., unchanged by REG-170) — those two fields were already covered by this same posture before this fix; only the `identityTables.*Table()` calls are new. |

### 4.14 → 2026-08-15, REG-177 (`LoginController` dynamic identity table-name resolution)

| Lead | Hits | Resolution |
|---|---|---|
| `LoginController.java:132` — the login-lookup `SELECT id, active, token_version FROM ... WHERE username = ? AND tenant_id = ?` | 1 | Rule **sql-string-building**, cleared safe (fingerprint `877b5e5187f8`). Same resolution as §4.13's REG-170 precedent: `identityTables` (an `IdentityPackTableNames`, resolved once in `LoginController`'s own constructor from its now-injected `CompiledModel`) replaces the previous hardcoded `identity_users` literal. |
| `IdentityPackTableNames.tryResolve`'s `catch (IllegalStateException)` | 1 | Rule **swallowed-security-exception**, cleared safe (fingerprint `c8629d8e9204`) -- but only after a real fix, not just triage: the sweep correctly caught that this catch block returned `Optional.empty()` silently, indistinguishable from the normal "identity pack not composed" no-op. Fixed by logging SEVERE before returning empty, matching REG-39's already-established "log loudly, then fail open" posture (`IdentityRoleLookup.tokenVersion`/`IdentityPermissionOverrideLookup.overridesFor`/`LoginController` all do the same). This specific catch only fires when `identity::User` IS composed but another identity-pack concept is missing -- a genuine platform inconsistency, distinct from the silent, correct no-op when `identity::User` is absent entirely. |

REG-177's OTHER new call sites (`IdentityRoleLookup.java`'s `ROLE_QUERY_TEMPLATE`/`TOKEN_VERSION_QUERY_TEMPLATE`, `IdentityPermissionOverrideLookup.java`'s `OVERRIDE_QUERY_TEMPLATE`, both now built via `String.formatted(...)` rather than `+` concatenation, plus `TenantAutoRegistrationRunner.java`'s already-cleared-shape splice) did **not** newly trigger the sweep at all -- either because their fingerprint happened to already exist in the allowlist for a different reason, or because the sweep's regex may not recognize a `.formatted(tableA, tableB, ...)` call the same way it recognizes `"..." + variable`. Not investigated further here (out of REG-177's own scope) -- worth a follow-up if a future sweep-tooling pass wants to confirm the pattern's `.formatted()` coverage is real, not a blind spot.
