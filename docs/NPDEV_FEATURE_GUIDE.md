...
| 4 | untrusted-extension panel | your own HTML/JS asset | your asset, hash-pinned into the app |
...
path 4 is any app declaring `metadata.untrustedExtensionEntrypoint`.
...
- **Untrusted-extension Java** — your own class, hash-pinned so the app refuses to run a
  modified one.
...
regeneration. Untrusted-extension Java when you need something the step vocabulary does not
have.
...
filesystem/network IO, process/system control, reflection, threads, scripting or detached
async work — but admission-checked is not sandboxed: memory and CPU are unbounded, and a
hostile plugin is not contained. See SEC-3 for the tracked upgrade path to real containment
(process isolation).
