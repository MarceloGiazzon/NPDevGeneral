...
| P2.1–P2.5 (DesiredSchema + SchemaDiff) | DONE | 2026-07-24 | e68a876 (desired) + this commit (engine) | **P2 complete.** P2.3 `Resolution` enum + `SchemaDiffItem` (itemKey/table/column/constraint/SafetyClass/before/after/resolution) + `SchemaDiff` (isEmpty/destructiveItems/hookEligibleItems). P2.4 `SchemaDiffEngine.diff(DesiredSchema, CurrentSchema)` — **rename-resolution FIRST** (table then column: a declared `renamedFrom` old→new is SAFE_RENAME, never drop+add), then table create/drop, column add(SAFE_ADDITIVE/NEEDS_BACKFILL/NEEDS_HOOK)/drop/type(via `TypeChangeMatrix`→SAFE_WIDEN/DESTRUCTIVE_NARROW_TYPE)/nullability(SAFE_RELAX; platform-loosened→tighten item). Destructive item keys reuse `SchemaDeltaItem.stableString()` verbatim (DROP_TABLE/DROP_COLUMN/NARROW_TYPE) for byte-identical tokens. Deterministic sort. Pure, wired nowhere. P2.5 RED→GREEN `SchemaDiffEngineTest` (15 cases: every SafetyClass + rename-vs-drop + table rename + widen/narrow + needs-hook + platform-loosened + empty + new-table). Full GATE-H2 green (incl. SchemaLifecycleExecutor* — no regression); GATE-PG unaffected (only unwired pure code added). FK/index diffing deferred (P0.2). NEXT: Phase 3 shadow parity (the crux). | P2.1+P2.2 DONE: `SafetyClass` enum (11 values, merges live + dead-lineage taxonomy) + desired records (`DesiredSchema/Table/Column/UniqueConstraint`, schemastate) + `DesiredSchemaFactory` (in `com.finalexec.db` — MUST live there for package access to the package-private `ColumnFacts`/`columnFactsFor`; sub-packages get no package access). Factory consumes `ColumnFacts` per the code directive, lower-cases names, carries platformManaged/requiredByModel/nullable/literalDefault/bond/renamedFrom. Pure, unwired. `DesiredSchemaFactoryTest` green (GATE-H2). **NEXT (P2.3–P2.5):** `SchemaDiffItem`/`SchemaDiff` + `SchemaDiffEngine` (rename-resolution FIRST; reuse `SchemaDeltaItem.stableString()` for destructive item keys; `TypeChangeMatrix` for widen/narrow) + the RED-first pure diff test. FK/index diffing deferred (P0.2 asymmetry). |
| P3.1–P3.4 (shadow parity) | **DONE — the crux, P3.4 go/no-go PASSED** | 2026-07-24 | 3014691, ba77b9a, ba8a9df, + this commit | **Phase 3 complete.** Parity driven 52→0 on schema-driven decisions; shadow assertion **default-on** in the test task (a standing invariant); GATE-H2 (all 27 scenarios) + GATE-PG (Postgres matrix + reader) green WITH assert on. 6 shadow fixes total (table-scoping, platform-NOT-NULL, type-qualification `qualifyTypeWithSize`, fingerprint-gating, refusal-skip, override-skips). **Owner-decided scoping (schema-driven only):** the probe skips when the live outcome is driven by state a schema-diff can't model — no fingerprint change; a refusal (REG-8 rollback/token/bond/crash); blanket destructive posture; whole-schema recreate; a mark-done no-op; an empty shadow diff (unique/FK/internal change — P0.2); or an ownership-gated table-drop (B7 orphan-left-alone). Each skip verified against a named matrix scenario via the assert-on run. Shadow wired via `beforeMigrate` wrapper, log-only+swallowed, behavior-preserving. **This is the go/no-go for Phase 4 — passed.** | *(history)* P3.3 progress: 52→13 via 4 correct shadow fixes (GATE-H2 + GATE-PG green, still log-only). (1) scope shadow to owned business tables — exclude `internalTables()`+`npdev_*`+`flyway_schema_history` (killed ~30 phantom DROP_TABLE); (2) platform columns always NOT NULL desired-side (killed `RELAX:id`); (3) reader qualifies types with size (mirror `qualifyTypeWithSize` → `VARCHAR`+50=`VARCHAR(50)`, killed all type-length NARROW noise); (4) gate the shadow on fingerprintChanged (mirror the live early-exit, killed fingerprint-match phantoms). **Remaining 13** (see OutsideRepo divergences-raw.txt) are the coarse-verdict oracle's hard tail — 8× shadow=SAFE/live=DESTRUCTIVE (destructive TEST scenarios where the live refuses for required-no-default/UNKNOWN/strategy reasons the schema-only shadow doesn't see; incl. NEEDS_HOOK→refusal mapping), 2× empty-diff/live=SAFE (change in a dimension the shadow doesn't model yet: uniques/FKs), 1× DROP_TABLE:gadgets rename-not-detected, 2× misc. These need per-scenario forensics + oracle refinement (the underspecified-oracle reality). P3.2 assert + P3.4 100%-gate NOT yet reached. **P3.1 DONE.** First live-executor edit: `beforeMigrate` split into a thin wrapper (unchanged name/visibility, so the 13 direct-driver tests still hit it) + `beforeMigrateDecision` (the original body). Wrapper snapshots `CurrentSchema` BEFORE the decision, runs `ShadowParityProbe` AFTER (on success OR refusal via finally), then rethrows any refusal unchanged. `ShadowParityProbe` (com.finalexec.db) builds `DesiredSchema` + runs `SchemaDiffEngine`, reduces both sides to a coarse Verdict (NO_CHANGE/SAFE/DESTRUCTIVE), and logs `SHADOW_DIVERGENCE` — **log-only, every path swallowed, cannot change behavior.** Full GATE-H2 + GATE-PG green (all 27 scenarios + PG matrix unregressed) — behavior-preserving confirmed. **NEXT (P3.2/P3.3):** flip the hard assertion (`npdev.schema.shadow.assert`) on in a shared test base, then drive parity to 100% (every divergence = a shadow bug: FK/index deferral, platform rules, internal tables the shadow doesn't model, mark-done/external/ahead short-circuits, the coarse-classification reduction). Log divergences in OutsideRepo `DIVERGENCE_LOG.md`. P3.4 gate = both matrices green WITH assert on. |
| P4.1–P4.9 (pass migration) | **DONE — Phase 4 COMPLETE. Every executor pass consumes the canonical SchemaDiff; REG-6 re-derivation debt collapsed.** | 2026-07-24 | 254f7c0 (classify), d8697ef+543cbbe (delta-report), 08f36fb+129991f (table renames), 7576e35+0e70f7d (column renames), 76c6aad+934a662 (widenings), c5e5871+68f92d5 (backfills) | **Phase 4 COMPLETE.** Both decision surfaces (`classify` → `ClassificationReducer` over the diff; `SchemaDeltaReport` → diff-derived itemization with byte-identical ack token) and all four mutation passes (table renames, column renames, type widenings, required-field backfills) now consume the ONE `SchemaDiff`. Each pass migrated prove-then-switch: a default-on equivalence assert proved the diff resolves the byte-identical work-list on H2+PG, then the bespoke re-derivation was retired, both gates green after every commit. Found+handled edges: classify's isolated table-rename blind-spot is now CLOSED (RENAME_DETECTED; live path unaffected — renames applied first); backfill required case-insensitive column matching (DesiredSchemaFactory lower-cases; manifest is model-case) + model-case resolution so DDL/messages stay byte-identical. Per-pass equivalence asserts retired after their switch; the standing guard is the behavior tests + the default-on shadow parity assert. Remaining known limit (NOT a re-derivation): FK/index diffing deferred (P0.2 asymmetry, plan P5.2). | *(history)* **P4.1 DONE — reducer proven+asserted equivalent to classify, live behavior UNCHANGED.** Drove the classify self-check to **0 divergences on H2 AND Postgres** via 3 faithful fixes: (a) `DesiredColumn.additiveEligible` (the manifest's `businessTableAdditiveColumns` membership = exactly what classify tests) drives add-safety + the reducer's NEEDS_HOOK→DESTRUCTIVE disambiguation (fixed `owner_ref` required-bond); (b) factory iterates `businessTableColumns().keySet()` (classify's desired-table set); (c) table-level scope — classify's classification is COLUMN-level, so the empty-manifest guard + all ownership-gated table-DROP decisions are scoped out of the self-check (`onlyTableDrops`/empty skipped; `DROP_TABLE→SAFE_ADDITIVE` baseline). Self-check now **default-on** alongside the shadow assert = standing invariant. **NEXT — the classify switch is now mechanical + de-risked:** replace classify's body with `ClassificationReducer.reduce(engine.diff(desired, scoped(current)), desired)` keeping the empty-manifest guard + table-level handling as pre-checks, remove the now-tautological self-check, guard with the 42-scenario matrix (H2+PG). Then P4.2 (delta-report/token byte-identical), P4.3-P4.7 (bond-refusal/relax/tighten/backfill), P4.6 (REG-40 remove the Part II tactical mechanism), P4.9 (renames+unique — needs the diff engine extended to model uniques, P0.2). One pass per commit; both gates + both asserts green after each. | *(history)* **P4.1 groundwork DONE, live behavior UNCHANGED.** `ClassificationReducer` (SchemaDiff→SchemaChangeClassification, pure, RED→GREEN unit-tested) + a gated behavior-preserving self-check (`ShadowParityProbe.compareClassification`, after the primary `classify()`; `-Dnpdev.schema.classify.check`) that empirically measures reducer-vs-classify equivalence. **Result: within ~3 divergences of classify** across the whole matrix, with two reducer mappings corrected to classify's ACTUAL semantics (any type change→TYPE_CHANGE_DETECTED not DESTRUCTIVE; new table→SAFE_ADDITIVE baseline). **Resume point — resolve these before switching classify to the reducer:** (1) `DESTRUCTIVE_DROP_TABLE` is ownership-gated (orphan→SAFE, proven concept-drop→DESTRUCTIVE) — thread an ownership signal in, it can't be reduced from pure schema; (2) one required-bond additive-eligibility mismatch (the diff's DesiredSchema/factory vs classify's `businessTableAdditiveColumns` for `widgets:owner_ref`). **Honest note:** each of P4.1–P4.9 is a mini-Phase-3 (measure→reconcile→verify→switch) on the live 3,318-line executor; ~1 session per 2–3 passes. Do NOT switch a pass until its self-check reaches 0 divergences on H2 + PG. |
| P5.1–P5.4 (finalize core) | **DONE (2026-07-25)** | 2026-07-24 → 2026-07-25 | d673c60 (P5.1/P5.3/P5.4), + this commit (P5.2) | **Corrected 2026-07-25:** this row previously read `NOT STARTED` while most of Phase 5 had in fact shipped — the ledger was never updated after `d673c60`, and P5.2 really had never been built. Now accurate. **P5.1/P5.3/P5.4 (d673c60):** retired the rebuild scaffolding (the dead `ShadowParityProbe.compareClassification` + the per-pass equivalence-assert properties, each of which had guarded exactly one switch and become tautological), flipped REG-6 to CLOSED FULLY in `NPDEV_OPEN_ITEMS_REGISTER.md` §1.6 + summary table and `DATABASES_AND_MIGRATIONS.md` §16, and kept the default-on shadow-parity assert as the standing invariant. **P5.2 (this commit):** `verifyExternallyManagedSchemaCompatible` was still column-shape-only (table/column existence + type) — an external schema matching every column NAME but violating the model's nullability or uniqueness assumptions verified "compatible" and failed later at runtime. It now also checks **nullability** (both fatal directions: model-requires-NOT-NULL vs live-nullable, and live-NOT-NULL-without-a-default vs model-optional — the `no default` qualifier prevents a false refusal, since a live `NOT NULL DEFAULT …` is safe for an optional field) and **unique constraints the model declares** (a live PK or unique over the same column set satisfies it; a live unique the model does not declare is tolerated — an external schema may be stricter than the model). Those facts are sourced from the canonical `CurrentSchema`/`DesiredSchema` pair, so this is no longer a second drifting notion of "does the live schema match" (REG-6). RED-first: 4 new scenarios in `SchemaLifecycleExecutorExternallyManagedTest` (10 tests total). **Still deferred:** FK/index verification — the manifest carries no explicit FK/index lists (P0.2 asymmetry), so the desired side cannot express them yet; tracked as P5.2-FK/G8. |
| P6.0, P6.4, P6.5 (Impact Report — CLI/ControlPanel surfaces) | **DONE — Phase 6 fully complete (P6.1–P6.3/P6.6 were already DONE above; this closes P6.0/P6.4/P6.5).** | 2026-07-24 | 39e821d (P6.0), f91c42f (P6.4), 16e84a3 (P6.5), 4d27ca4 (docs) | `SchemaImpactFacade.forLiveDatabase` (P6.0) — the ONE public entry point both remaining surfaces share, so they can never disagree. P6.4: `REPORT_ONLY` lifecycle mode (`-Dnpdev.schema.lifecycle.mode=REPORT_ONLY`) computes+prints the impact report and exits 0/2/3 BEFORE any fingerprint read/claim/DDL/history write; `Build-NpdevApp.ps1 -ImpactOnly` + an emitted `_ops\Impact-Only.ps1` drive it end to end (deviated from the plan's literal text: `Build-NpdevApp.ps1` never builds/runs the jar itself in ANY mode — `-ImpactOnly` delegates to the emitted `Build-App.ps1`+`Impact-Only.ps1` ops-toolbox pair instead of inventing an inline build/run path). P6.5: `SchemaImpactController` (SUPERUSER-gated, copies `SchemaAcknowledgmentController`'s pattern exactly) — `GET /impact` (JSON) + `/impact/view` (HTML). All three gates green throughout. |
| P7.1–P7.7 (conversion hooks) | **DONE — Phase 7 fully complete, rule-6 owner-approved, live-proven TWICE on a real generated app (once same-day, once again in the `SER_FINAL_CLOSURE_PLAN.md` closure session).** | 2026-07-24/25 | 4c2e162 (P7.1+P7.2), fbf3319 (P7.3+P7.5), e165c99 (P7.4), 9e07aa2 (P7.6); closure-plan hardening: 98d23e5 (G1 jar-load proof), 79f7277 (G3 backfill e2e), 154a938 (G4 splitter), 2c769de (G5+G6), 3d51477 (G7) | `conversion-hook.schema.json` + `ConversionHookEmitter` (generation-time validation, sibling of `SchemaRealizationEmitter`). `ConversionHookRunner` (`com.finalexec.db`, NOT `...schemastate` as the plan sketch said — that subpackage has no access to `ShadowParityProbe.scopeToOwnedBusinessTables`) implements all 7 rules; wired into `beforeMigrateDecision` right before the destructive decision. Rule 6 ("authoring the hook IS the acknowledgment") needed two supporting fixes: `SchemaLifecycleExecutor` treats an empty post-hook `SchemaDeltaReport` as fully-resolved/no-token-needed, and `ShadowParityProbe.compareAndLog` gained a `conversionHooksApplied` exemption. `ImpactReport` threads hook claims (`HOOK_CLAIMED`, `HOOK: <id>` rendering, P7.4). **Closure-plan hardening (`SER_FINAL_CLOSURE_PLAN.md`, 2026-07-25, ADR-0008 rule-6 sign-off APPROVED):** a finding-#1 fix landed BEFORE the closure plan (verify now runs INSIDE the hook's own transaction, not after a separate commit — `fce3eb1`); the closure plan then added a jar-loading proof (green first try — no bug found), a backfill end-to-end `beforeMigrate` proof, a comment/quote/dollar-quote-aware SQL splitter rewrite, per-hook-atomicity + H2-DDL-mix operator docs/warnings, and manifest-preferred engine detection. Live-proven on a real packaged jar TWICE: the original split-column (`priceCents`→`priceDollars`) proof, then a second round (`weightGrams`→`weightCategory`, a CASE-expression conversion with a real `--` comment in `convert.sql`) exercising every one of the above fixes together — see `D:\WorkSpace\NPDev\NPDev_General__OutsideRepo\schema-engine-rebuild\P7.7-live-proof\EVIDENCE.md`. All three gates green throughout, both sessions. |
| P8.1–P8.3 (type-conversion proposals) | **DONE.** | 2026-07-24 | a9e7aeb | `ProposedConversionSql.forNarrowing` (pure) drafts a copy-convert script (ADD temp column → UPDATE via `SUBSTRING`/`CAST` → DROP old → RENAME temp into place) for a convertible `DESTRUCTIVE_NARROW_TYPE` item; `null` for an `INCOMPARABLE` cast. Wired into `ImpactReportJson`/`ImpactReportText` (previously hardcoded `null`). Never auto-run — explicit non-goal, documented. |
| P9.1–P9.3 (retire dead lineage, docs) | **DONE.** | 2026-07-24 | d0b5a4a (P9.1), fab628d (P9.2), b62ea63 (P9.3) | P9.1: deleted the 12-class `com.finalexec.npdev.migration.*` lineage after a full prove-then-delete repo grep (zero main-code references outside the deleted package); the plan's named "generator-side tests to also delete" turned out to guard an unrelated, already-closed retirement — left untouched. Removed `FinalAppAssembler`'s `db/migration-plans/` skip entry. P9.2: `MigrationPlanEmitter` kept (different design, still useful) and re-documented against `-ImpactOnly`. P9.3: `DATABASES_AND_MIGRATIONS.md` §12/§20 rewritten, REG-40 marked CLOSED, new knowledge card, one full live rebuild proof. |
...
everything this programme closed (REG-40, all-or-nothing freedom) and **narrow — do not delete —**
the `ExternallyManaged` line to the part that is still true (FK/index only; nullability + declared
uniques were closed at P5.2 on 2026-07-25); §20 rewritten. `NPDEV_OPEN_ITEMS_REGISTER.md`: REG-6 CLOSED, REG-40 CLOSED. Add a
...
- **`ExternallyManaged` column-shaped-only verification** — closed at P5.2 for **nullability + declared
  uniques** (2026-07-25). **FK/index verification is still deferred** (the manifest has no explicit
  FK/index lists — the P0.2 asymmetry), so do NOT delete the `§15` limitation line wholesale: narrow it
  to FK/index instead.
