...
| P6.1–P6.6 (Impact Report) | **PARTIAL — engine + Surface 1 + docs DONE; CLI/UI surfaces HELD by owner** | 2026-07-24 | 10a48d7 (P6.1), f584cba (P6.2), 8e84c27 + 1742002 (P6.3), 1742002 (P6.6) | **DONE:** P6.1 `ImpactReport` (read-only row-count probes per item type + worst-item-wins verdict, 5s timeout, never-throws — `ImpactReportH2Test`). P6.2 `ImpactReportText` + `ImpactReportJson` + `NPDevContract/schemas/impact-report.schema.json` (pure/deterministic — `ImpactReportRenderersTest`). P6.3 Surface 1: writes `runtime-data/impact-reports/<ts>-<from>-<to>.json` (retain 10) + prints the text on EVERY upgrade boot (hooked at the single common point after the first classify), and the destructive refusal message now LEADS with the impact table while keeping the byte-identical token + itemization + docs pointer (DestructiveItemizationTest green). P6.6 `docs/IMPACT_REPORTS.md` + cross-link. **HELD by owner decision (2026-07-24):** P6.4 (REPORT_ONLY mode + `-ImpactOnly` CLI + exit codes) and P6.5 (SchemaImpactController + ControlPanel page) — user-facing surfaces to shape with the owner. **Phase 7 HELD** (rule-6 sanctioned-destruction owner sign-off). |
