...
| **REG-59** | Found while re-verifying the REG-58 fix against WmsOffice's real, partially-migrated database, 2026-07-28 (`docs/POST_PUBLIC_PLAN.md` P4.3 re-verification) | MEDIUM | **FILED, not fixed — pre-existing, confirmed on live data, not caused by the REG-58 fix.** `DestructiveRecreationPass.executeNarrowTypeDropAndRecreate`'s `ALTER TABLE ... ADD COLUMN <safeColumn> <safeType>` never re-applies `NOT NULL`, even when the model declares the field required and the column being recreated was `NOT NULL` before the drop. Confirmed via a fresh `REPORT_ONLY` impact check run against WmsOffice's live database after the REG-58 fix landed: the **6 columns already narrowed by the ORIGINAL, pre-REG-58 code** before it crashed (`areas.nome`, `armazems.nome`, `documento_fiscals.chave`, `documento_fiscals.numero`, `entidades.nome`, `historico_movimentacaos.evento`) now show a new `NEEDS_HOOK NOT NULL -> NULL` drift item each — meaning the live column is nullable while the model still declares it required. **This is not new damage from the REG-58 fix**: those 6 columns were narrowed by the unmodified original code, before this session touched `DestructiveRecreationPass.java` at all; REG-58's fix only unblocked the crash on item 9 (`identity_password_reset_tokens.token_hash`), it did not touch this code path's `ADD COLUMN` statement. 15 of the 18 items remaining after the REG-58 fix (everything except `identity_users.email`, `produtos.ncm`, `workspace_menus.required_role`, `workspace_preferences.user_id` — confirmed by cross-checking `V1__npdev_schema_realization.sql`'s desired-schema DDL) are also declared `NOT NULL`, so completing the migration will compound the same drift onto those columns too — accepted as a known, tracked gap rather than blocking the migration retry (see the `AskUserQuestion` decision recorded alongside this filing: proceed now, fix constraint fidelity separately). **Why this isn't a trivial one-line fix:** the drop-and-recreate path is deliberately "data acknowledged lost, recreate empty" (see the method's own comment), so for a table with existing rows (`areas` has 25, `armazems` has 5, `entidades` has 7), a plain `ADD COLUMN ... NOT NULL` with no `DEFAULT` would itself fail (existing rows would have no value to satisfy the constraint) — the real fix needs either a placeholder `DEFAULT` (e.g. `''` for VARCHAR) so existing rows can satisfy `NOT NULL`, or an add-nullable/backfill/`ALTER COLUMN SET NOT NULL` three-step sequence, and a decision on what placeholder value is defensible to write into a column whose real data was just intentionally destroyed. **Needs:** a RED-first test asserting the recreated column's `NOT NULL`-ness matches the model's declared `required` (or the pre-drop live column's nullability) for a table with existing rows, then a fix choosing one of the above strategies. |
