...
| ~~**REG-39**~~ | — | HIGH | **DONE for WmsOffice (2026-07-24); platform hazard noted.** Symptom: WmsOffice login failed with `invalid_credentials` for every credential, because `LoginController` (+ `JwtSigner`/`PasswordResetController`/`ControlPanelTenantUsersController`) unconditionally `SELECT … token_version FROM identity_users` and the column did not exist → the query threw and was swallowed. **True root cause (NOT a generator/platform-code bug):** the generator reads the identity pack from wherever the app's `packs[].$ref` resolves, and WmsOffice carried its **own private, drifted copy** of the built-in identity pack at `AppGen/apps/_official/WmsOffice/definition/packs/identity/pack.json` — a pre-LNCH-4 snapshot missing both the `User.tokenVersion` field AND the entire `PasswordResetToken` concept. So the generated `identity_users` schema legitimately omitted `token_version`. The **platform** pack (`NPDevContract/packs/identity/pack.json`) has been correct since LNCH-4 (commit 7cd804e); the kernel REG-23 read (`IdentityRoleLookup.tokenVersion`) is resilient (catches SQLException→0), so REG-23 itself was never broken. **Fix (layer 2, app-scope):** synced WmsOffice's private pack copy to the platform pack (adds `tokenVersion` + `PasswordResetToken`). Regeneration now emits `token_version` (and `identity_password_reset_tokens`); **verified live on a FRESH DB** — login returns a valid JWT and the ARCH-upload P6 upload→download round-trip succeeds. **Platform hazard (open follow-up):** AppGen apps pinning a private copy of a *built-in* pack drift silently from the platform and break against newer platform code (here: login). Consider a drift guard (a gate that diffs each app's private built-in-pack copies against `NPDevContract/packs/*` and fails on divergence), or sourcing built-in packs from the platform instead of copying. Not built here. |
| **REG-40** | — | MED | **OPEN (filed 2026-07-24).** The additive/repeatable migration `R__npdev_schema_additive_columns.sql` only ever emits `ALTER TABLE … ADD COLUMN`/`ADD CONSTRAINT` (403 ALTERs, **0 CREATE TABLE**). When a **new concept/table** is added to a model that is then re-deployed against an **existing** DB, the versioned `V1` CREATE-TABLE migration has already run and will not re-run, so the new table is never created — and the additive `ADD COLUMN` against it fails at boot with `Table "…" not found`, refusing the app. Discovered live: syncing WmsOffice's identity pack added the `PasswordResetToken` concept (`identity_password_reset_tokens`); an existing WmsOffice DB refused to boot because the additive migration tried to `ADD COLUMN` to a table it never created. Fresh DBs are unaffected (V1 creates everything). Fix: the additive/repeatable migration must `CREATE TABLE IF NOT EXISTS` (idempotent) for every model table **before** its `ADD COLUMN`s, so new tables self-heal on upgrade the way new columns already do — OR route new-table creation through `SchemaLifecycleExecutor`'s additive path. Orthogonal to REG-38 (that was constraint idempotency on *existing* tables). |
