...
| **REG-56** | Found while building the CORE C-3 durable-workflow demo, 2026-07-28 (`docs/POST_PUBLIC_PLAN.md` P4.2) | MEDIUM | **FILED, not fixed — reproduced precisely, root cause not yet traced.** A `capabilityCall` step (the demo's `notify-approval`, capability `notification`/operation `send`, adapter `notification-inproc`) executed as part of resuming an `awaitEvent`-parked flow throws `CAPABILITY_FAILED` (log: `{"event":"npdev.flow.outcome",...,"outcome":"failure","status":"CAPABILITY_FAILED"}`, no further detail surfaced by `ExecutionQueryController`'s response shape) **specifically when the resuming process is a genuinely different JVM from the one that started the flow** (real process kill + restart, H2Local file-backed engine) — **the identical capability call succeeds** when submit-then-publish happens inside one continuously-running process (confirmed twice: once via `docs/POST_PUBLIC_PLAN.md`'s own durable-resume rehearsal reasoning, once via a live side-by-side repro — same model, same adapter, same capability call, only the process-restart step differs). Rehydration itself is confirmed correct up to this point: `GET /api/executions/{id}` and `/api/executions/waiting` both show the instance correctly rehydrated as `WAITING_EVENT`, `resumeAttemptCount=0`, `nextEligibleResumeAtEpochMs=null` (eligible) in the fresh process **before** publishing; the resume attempt itself (fired synchronously from `KernelRunner.publishEventInternal` → `ResumeCoordinator.resumeWaitingExecutionsFor`, confirmed in the log ~8-10s after publish, not instant) is what fails, moving the instance to `FAILED_PERMANENT`. Candidate hypotheses, **none yet confirmed**: (a) another instance of REG-55's exact shape on a different adapter's overload set (`notification-inproc`'s `send` method(s) not yet inspected for overloads); (b) some plugin/adapter registry or realization-summary state that a fresh JVM populates lazily on the FIRST flow-start dispatch but not on a RESUME-only dispatch path, so a rehydrated flow's first capability call after restart never sees it warmed. **Worked around, not fixed, for the demo:** `NPDevSamples/durable-workflow-demo`'s flow was simplified to drop the `notify-approval` capability call (`await-approval` returns directly), sidestepping this bug rather than blocking CORE C-3 on it — the capability-call-after-resume case is a real, separate, open gap this filing exists to not lose track of. Needs a RED-first repro isolating the resume-path capability dispatch from the flow-start capability dispatch before attempting a fix. |
