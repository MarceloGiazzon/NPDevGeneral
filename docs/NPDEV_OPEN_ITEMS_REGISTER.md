...
> **STATUS: HISTORICAL / ARCHIVED-IN-PLACE (2026-07-29, docs/REMEDIATION_PLAN.md R-P1).** The 2.E
> ledger migration is complete: all 64 tracked ids (REG-1 through REG-63, plus REG-16-resid) now
> live as structured YAML at `ledger/items/*.yml`, rendered to `docs/OPEN_ITEMS.md`. **That pair —
> not this document — is the source of truth for current status.** This file is kept, unedited for
> status going forward, purely as the historical investigation narrative: root causes, fix
> rationale, verification detail, and the corrections/retractions along the way are genuinely
> valuable and were deliberately NOT bulk-copied into the terser YAML `detail` fields (see
> `ledger/README.md`) — every `ledger/items/<id>.yml` points back here via `legacyDetailRef`, and
> the anchors below (`#reg-N`) are why the file stays in place rather than moving to `docs/archive/`.
> To open a new item, add a `ledger/items/<ID>.yml` file directly — do not add a row here.
>
...
| New item | From | Sev | Status |
|---|---|---|---|
| ~~**REG-54**~~ | T2.B.4 file-split verification, 2026-07-27; fixed 2026-07-28 (`docs/NEXT_EXECUTION_PLAN.md` P3.4) | LOW | **DONE (2026-07-28).** While splitting `SchemaLifecycleExecutor.java` (`docs/DSL2_AND_DECOMPOSITION_PLAN.md` §2.B.4), `worse(SchemaChangeClassification, SchemaChangeClassification)` and `hasTypeChange(...)` (both `private static`) were found to have zero callers anywhere in `com.finalexec.db` — confirmed by direct grep, not by test failure. Re-confirmed zero callers repo-wide (not just `com.finalexec.db`) before deleting. Both methods deleted; the dangling `{@link #hasTypeChange}` javadoc reference in `normalizeSqlType`'s doc-comment, and the three test files' doc-comments (`SchemaLifecycleExecutorNormalizeSqlTypePostgresAliasTest`, `SchemaLifecycleExecutorTypeChangeLengthPrecisionGapTest`, `SchemaLifecycleExecutorVarcharTypeNormalizationTest`) that referenced `hasTypeChange()`/`classify()` as if still live, updated to describe the historical pre-SER-P4.8 behavior instead. `NPDevRuntimeHost` `SchemaLifecycleExecutor*` test suite green after the deletion. |
| ~~**REG-55**~~ | T2.B.4 live rehearsal, 2026-07-27; root cause found independently during T2.B.5's rehearsal, same day; fixed 2026-07-28 (`docs/POST_PUBLIC_PLAN.md` P4.2) | MEDIUM | **DONE (2026-07-28).** First seen during the 2.B.4 rehearsal (additive-column change against `AppGen/apps/simple-product-h2local`, H2Local engine): a REST write threw `IllegalStateException: Ambiguous sandboxed plugin operation save ... PostgresPersistenceCapabilityAdapter`. Seen again during 2.B.5's rehearsal on a different sample, then a THIRD time — for real, blocking real work, not a rehearsal aside — while building the CORE C-3 durable-workflow demo sample (`NPDevSamples/durable-workflow-demo`), which is what triggered actually fixing it rather than deferring further. **Root cause:** `SandboxedPluginExecutionEngine.resolveOperation` matched a candidate handler method by name + **parameter count only**, so `PostgresPersistenceCapabilityAdapter`'s two 2-argument `save` overloads (`save(Object,Object)` and `save(TenantScope,Object)`) always threw "Ambiguous," regardless of the actual runtime argument types — in the real call path, `adaptCallForHandler` enriches a 1-arg save into 2 args by prepending the concept name as a **String**, which is never a `TenantScope`, so exactly one overload was ever actually legal. **Fix:** `resolveOperation` now disambiguates same-name/same-argCount candidates by checking which ones the actual argument values are assignable to (boxing primitives first); falls back to the original "not found"/"ambiguous" errors only when that check doesn't narrow to exactly one method. **RED→GREEN:** new `SandboxedPluginExecutionEngineTest#disambiguatesOverloadsBySameArgCountByActualArgumentType`, reproducing the exact overload shape with a `TenantScopeMarker` fixture type — confirmed RED against the pre-fix code (threw the ambiguous-operation `IllegalStateException`, 1/7 failing in this class), GREEN after (7/7). Full `NPDevRuntimeHost` suite 404/0 (403 baseline + this test), no regression. |
| ~~**REG-56**~~ | Found while building the CORE C-3 durable-workflow demo, 2026-07-28 (`docs/POST_PUBLIC_PLAN.md` P4.2); root-caused and fixed 2026-07-28 (`docs/NEXT_EXECUTION_PLAN.md` P3.2) | HIGH | **DONE (2026-07-28).** **Both filed hypotheses were refuted by tracing code, not guessed away:** (a) `notification-inproc`'s `send` has exactly one overload — `Object send(Object payload)` — and, unlike REG-55's adapter, implements `CapabilityAdapter` directly, so dispatch never reaches `resolveOperation`'s overload-matching at all; (b) the capability registry is built eagerly inside the `CapabilityRegistry` `@Bean` factory method itself (`NpdevCapabilityBindingConfig`), and `KernelRunner`'s bean depends on it — fully populated before `KernelRunner` (and therefore any resume trigger) can exist. **Actual root cause:** a real permission-context bug, confirmed live via `PermissionDebugConfig`'s own debug log — the resumed flow's `capability.invoke` check ran as `roles=[user]` (denied; only `role=admin` is granted `capability.invoke`), moments after the SAME request's `event.publish` check had run as `roles=[admin]` for the SAME actor/tenant. Three call sites each built a resume `ExecutionContext` a different, wrong way: the event-driven path (`KernelRunner.publishEventInternal` → `ResumeCoordinator.resumeWaitingExecutionsFor`) used the PUBLISHER's own context (denying resume when the publisher lacks a capability the flow needs, or over-granting when the publisher happens to be an admin — accidentally "working" in the same-process case this filing's first repro compared against, which is why it never reproduced there); the scheduled sweep (`resumeAllWaitingExecutions`) used `ExecutionContext.of(tenantId, actorId)`, which always defaults role to `USER` regardless of what role the flow was actually submitted under; the public `resumeFlow` port method used `ExecutionContext.anonymous()`. None preserve the flow's own authorization level, because `FlowInstance` never persisted roles in the first place — only `tenantId`/`actorId`. **Fix:** new `ExecutionContext.resuming(tenantId, actorId)` (`NPDevKernel/kernel/.../ExecutionContext.java`), granting the trusted resume-level role — mirroring the ADMIN trust `ExecutionContext.system` already grants the cron scheduler (LNCH-12: a background continuation of already-authorized work is not a new privileged request) — while keeping the flow's own actor for audit traceability. Wired into all three call sites (`ResumeCoordinator`'s event-driven path and scheduled sweep, both in `NPDevKernel/kernel/.../ResumeCoordinator.java`); `resumeWaitingExecutionsFor`'s now-unused caller-supplied-context parameter removed, updating its four call sites (`KernelRunner`, `EmitEventStep`, `ScheduleEventStep`). **RED→GREEN, both freshly reproduced on this checkout:** the `notify-approval` capabilityCall step (dropped by the original workaround) was added back to `NPDevSamples/durable-workflow-demo`'s model, reproducing `CAPABILITY_FAILED` on the first live run after a real kill+restart (log confirmed `roles=[user]`, denied); with the fix, the same scenario shows `roles=[admin]` and `NPDEV-PLUGIN-SANDBOX :: phase=finish status=SUCCESS`, **3/3 clean runs** of `run-durable-resume-demo.ps1` with the capability step present. Plus `ExecutionContextResumingTest` (3/3 green) and the full `NPDevKernel:kernel` suite (163/163, no regression). **Completion criterion met:** the demo re-adds the `capabilityCall` step and passes across a real restart. |
| ~~**REG-57**~~ | Found while building the CORE C-3 durable-workflow demo, 2026-07-28 (`docs/POST_PUBLIC_PLAN.md` P4.2); re-rated HIGH and fixed 2026-07-28 (`docs/NEXT_EXECUTION_PLAN.md` P3.1) | HIGH | **DONE (2026-07-28).** **Root cause, traced to a specific mechanism, not a guess:** ack-ordering was eliminated first, by code, not inference — `flowInstanceStore.update(waiting)` (`KernelRunner.java`'s `WAITING_EVENT` branch) is a plain blocking call that strictly precedes the method's `return`, on a fully synchronous, single-threaded servlet call chain all the way to `ResponseEntity` construction in the generated `FlowExecutionController` (no thread hop, no queue, no `@Async`, no batching, no decorator between the kernel and the JDBC statement). That leaves physical durability: H2's MVStore defaults to a 500ms `WRITE_DELAY`, buffering committed writes in memory before flushing to disk, and this parameter was **not set anywhere in the repo** (confirmed by a repo-wide grep) — `JdbcFlowInstanceStore.update` relies entirely on Hikari's default autocommit, with no explicit flush/checkpoint. A hard kill inside that up-to-500ms window loses however many commits landed since the last flush, even though each JDBC call had already returned success. **Correction to this filing's original text:** the surviving row (`status=RUNNING, currentStepIndex=1`) is NOT the initial `save(initialInstance)` checkpoint (that call produces `currentStepIndex=0` per `FlowInstance.start`) — it is the checkpoint written by the per-step `progressRecorder` *after* step 0 completed. The demo flow has steps 0 validate / 1 save-expense / 2 emit-event / 3 approval-gate, so **a contiguous tail of at least three commits was lost together** (index→2, index→3, and the final `markWaiting`), not one missed statement — a signature consistent with a time-windowed buffer loss, not a single dropped write. **Fix:** `;WRITE_DELAY=0` added to the H2 JDBC URL construction (`UserDatabaseDefinitionLoader.jdbcUrl`, both `H2_LOCAL` and `H2_SERVER` branches — the only production call site; a repo-wide grep found no other URL-construction site), forcing a physical flush on every commit. Applied at the URL level rather than "the flow-instance write path only" per the plan's stated preference, because the evidence shows the SAME gap exposed every other JDBC store on this DataSource (`JdbcEventStore`, `JdbcBusinessConceptStore`, etc. — the lost step-1 write was a business-concept save, not a flow-instance write). **Postgres parity, confirmed as the plan asked:** unaffected — `COMMIT` is synchronous to WAL there; no `WRITE_DELAY`-equivalent buffering parameter exists or is needed. **RED→GREEN, both freshly reproduced on this checkout, not just cited from the original filing:** with the fix reverted and the demo's workaround sleep removed, `run-durable-resume-demo.ps1` reproduced the exact failure fresh (step 6 timeout, execution never reached `OK`); with the fix restored and the sleep still removed, the demo passed **3/3** (matching the original filing's 3/3-fail cadence, now 3/3-pass). Plus a fast unit regression guard, `UserDatabaseDefinitionLoaderWriteDelayTest` (2/2 green), asserting the URL always carries `WRITE_DELAY=0` for both H2 engines. **Completion criterion met:** the 5-second sleep is deleted from `run-durable-resume-demo.ps1` (with a comment recording the fix), and the demo passes without it. | Killing the process (`Stop-Process -Force`, a hard kill, not a graceful shutdown) **too soon** after the HTTP response for `POST /api/flows/{name}/execute` already reports `status: WAITING_EVENT` can leave the on-disk H2Local database's `npdev_flow_instance` row at an **earlier** persisted checkpoint than what the response told the caller — reproduced 3/3 times with near-zero delay (row stuck at `status=RUNNING, currentStepIndex=1`, i.e. the initial `flowInstanceStore.save(initialInstance)` checkpoint, never advancing to the `flowInstanceStore.update(waiting)` call `KernelRunner.executeFlowInstance` makes at the `WAITING_EVENT` branch — `KernelRunner.java` around the `if (failure.getStatus() == ExecutionStatus.WAITING_EVENT)` block), and confirmed **absent** with a 5-second delay inserted between "response received" and "kill" (1/1 clean). `JdbcFlowInstanceStore.update` is straightforward per-call synchronous JDBC (`try (Connection connection = dataSource.getConnection())`, no batching/async/decorator layer found — only two `FlowInstanceStore` implementations exist repo-wide, in-proc and this JDBC one), so the gap is presumably below the JDBC layer: either H2's own file engine buffering a page write that a hard kill doesn't force to disk despite the JDBC call returning, or the connection pool/auto-commit interaction not actually forcing a physical flush per statement for this specific H2 URL (`jdbc:h2:file:...;MODE=PostgreSQL;DATABASE_TO_LOWER=TRUE;DB_CLOSE_ON_EXIT=FALSE` — no explicit sync/flush-related H2 URL parameter). **Not yet root-caused to a specific mechanism** — filed as a real, reproducible, timing-shaped gap rather than a guess at a fix. **Worked around, not fixed, for the demo:** `run-durable-resume-demo.ps1` inserts a short deliberate delay before killing the process, with this finding cited in a comment, so the demo reliably exercises the intended durable-resume path rather than this race. **This is a real gap worth someone's dedicated attention**: if the platform's core "durable workflow" guarantee can be defeated by a hard kill landing in the wrong few-hundred-millisecond-to-few-second window after a client already saw a success response, that is a correctness gap in the platform's own flagship differentiator, not a demo inconvenience. Needs: a RED-first repro that pins the exact layer (H2 file engine vs. connection pool vs. something in `KernelRunner`), ideally by testing whether the same race reproduces against Postgres (would point at the kernel/JDBC layer) or is H2Local-specific (would point at H2's own file engine/buffering). |
| ~~**REG-58**~~ | Live, real destructive migration on WmsOffice's production database, 2026-07-28 (`docs/POST_PUBLIC_PLAN.md` P4.3 re-verification) | HIGH | **DONE (2026-07-28).** WmsOffice's real, user-acknowledged destructive migration (26 `DESTRUCTIVE_NARROW_TYPE` items, itemized per `docs/LNCH1_HARDENING_PLAN.md`'s Phase 4/6 mechanism) crashed **8/26 items in**, on `identity_password_reset_tokens.token_hash` (`VARCHAR(255)` → `VARCHAR(64)`), with `org.h2.jdbc.JdbcSQLSyntaxErrorException: Column may be referenced by "public.ux_identity_password_reset_tokens_token_hash"; ALTER TABLE identity_password_reset_tokens DROP COLUMN token_hash [90083-224]`. The partially-migrated database file was backed up immediately, before any further action. **Root cause:** `DestructiveRecreationPass.executeNarrowTypeDropAndRecreate` issued a plain `ALTER TABLE ... DROP COLUMN` with no regard for a unique index/constraint still referencing that column. This is not a one-off: every model field declared unique gets a **tenant-scoped, composite** bootstrap index from `SchemaRealizationEmitter` — `ux_<table>_<column> ON <table> (tenant_id, <column>)`, per the generated `V1__npdev_schema_realization.sql` — and H2 (Postgres identically) refuses to silently drop a column that is only ONE of a composite index's columns (it happily auto-drops a single-column index sharing the dropped column, which is why an initial single-column repro attempt did not reproduce the failure at all — the composite `(tenant_id, column)` shape was the load-bearing detail). Several other columns in the SAME batch (`identity_users.username`/`email`, `produtos.codigo`, `ruas.codigo`) are equally likely unique-constrained business keys and would have hit the identical crash later in the same run. **Fix:** new `dropIndexesReferencingColumn` (portable `DatabaseMetaData#getIndexInfo`, not a naming-convention assumption) finds and drops every index touching the narrowed column before the `DROP COLUMN`/`ADD COLUMN` pair, via `DROP INDEX IF EXISTS` falling back to `ALTER TABLE ... DROP CONSTRAINT IF EXISTS`. Deliberately does **not** recreate the constraint itself — `UniqueConstraintPass.applyUniqueConstraints` already runs on every boot's `afterMigrate` and idempotently re-adds any declared unique constraint it finds missing, so recreating it here would be redundant with, and could race, that existing pass. **RED→GREEN:** new `DestructiveRecreationPassNarrowTypeUniqueColumnTest` (file-backed H2, `MODE=PostgreSQL;DATABASE_TO_LOWER=TRUE;DB_CLOSE_ON_EXIT=FALSE` — matching `UserDatabaseDefinitionLoader`'s real H2Local URL exactly) reproduces the identical exception byte-for-byte against the real composite-index shape with the fix's core call disabled; passes (2/2) with it restored. Full `NPDevRuntimeHost` suite 406/0, no regression. **Not yet closed end-to-end on WmsOffice itself, as of this filing** — since superseded: REG-59 records that the live database was recovered (backfilled, `ALTER COLUMN ... SET NOT NULL`, `Impact-Only.ps1` → `verdict: SAFE`, boot succeeded, `/actuator/health` → `UP`). See REG-59. |
| ~~**REG-59**~~ | Found and resolved-on-live-data while re-verifying the REG-58 fix against WmsOffice's real, partially-migrated database, 2026-07-28 (`docs/POST_PUBLIC_PLAN.md` P4.3 re-verification) | MEDIUM | **DONE for WmsOffice, as a live-data recovery record only.** This row covers the manual recovery performed against WmsOffice's real database; it does not cover the platform gap that recovery exposed — that gap is filed separately, OPEN, as **REG-61** (see below), so it stays visible to anyone scanning for open work. `DestructiveRecreationPass.executeNarrowTypeDropAndRecreate`'s `ALTER TABLE ... ADD COLUMN <safeColumn> <safeType>` never re-applies `NOT NULL`, even when the model declares the field required. Not caused by the REG-58 fix — the 6 columns the ORIGINAL, pre-REG-58 code had already narrowed before it crashed (`areas.nome`, `armazems.nome`, `documento_fiscals.chave`/`numero`, `entidades.nome`, `historico_movimentacaos.evento`) exhibited the identical drift. **Corrected understanding vs. this filing's first draft:** the platform's `BackfillPass.applyRequiredFieldBackfills` (`afterMigrate`, LNCH-1 Phase 5, `docs/SCHEMA_EVOLUTION.md#new-required-fields`) DOES catch this on the very next clean boot — it re-diffs live-vs-desired schema fresh every boot, sees these columns as `TIGHTEN_NOT_NULL`/`ADD_REQUIRED_COLUMN` items, and **refuses to boot** rather than silently leaving them nullable, unless the model declares a literal `default` to backfill with. So the "silently loses NOT NULL forever" framing in this filing's first draft was wrong: the exposure window is only *during* a crashed/interrupted boot, not permanent. **The real, deeper gap this surfaced:** the sanctioned recovery mechanism (declare a literal `default`, backfilled via a single `UPDATE ... SET col = ? WHERE col IS NULL` bound parameter) cannot satisfy a `UNIQUE` constraint across more than one existing row — a flat literal writes the *same* value into every null row, so for a tenant-scoped unique column with >1 affected row (confirmed live: `identity_roles.name`, 5 rows; `identity_users.username`, 6 rows — both back a `CREATE UNIQUE INDEX ux_..._<col> ON <table> (tenant_id, <col>)`), applying the declared default would only trade "boot refuses, no default" for "boot refuses, duplicate key" once `UniqueConstraintPass` re-adds the constraint. There is no model-level `default` mechanism expressive enough for "a value unique per row" (e.g. derived from the row's own id). **Resolved on WmsOffice's live database via direct, out-of-band SQL** (not a model or platform-code change): backfilled all 18 blocked columns — a flat placeholder (`'MIGRATED'`) for the 16 non-unique ones, a per-row-unique placeholder (`'role-' \|\| CAST(id AS VARCHAR(36))` / `'user-' \|\| CAST(id AS VARCHAR(36))`) for the 2 unique ones — then `ALTER COLUMN ... SET NOT NULL` directly, so the next boot's diff sees every column already converged (no `BackfillPass` involvement, no model change needed). Verified via a fresh `Impact-Only.ps1` run: `verdict: SAFE`, `0 destructive / 0 attention`, then a real boot succeeded (`/actuator/health` → `UP`). **Real consequence, already covered by the original acknowledgment, not new:** `identity_users.username/email/display_name`, `identity_roles.name`, and `usuarios.senha_hash` for WmsOffice's then-existing 6 users / 5 roles are now placeholder values, not their original real data — the destructive DDL had already committed (auto-commit) before the backfill-refusal was ever reached, so this was already true before the manual recovery; recovery only unblocked the boot, it did not cause additional loss. Login credentials will need to be re-established (see follow-up task). **Platform-level gap filed as REG-61 (OPEN, HIGH), not covered by this row's DONE marker:** (a) `executeNarrowTypeDropAndRecreate` should preserve original `NOT NULL`-ness like the fix above did manually, so a table with zero existing rows doesn't need the full backfill dance at all; (b) the literal-default backfill mechanism has no way to express a per-row-unique default for a `UNIQUE`-constrained required column with existing rows — document this as an explicit refusal case (like the bond-column refusal) rather than let it surface as a second, differently-worded boot failure after the first is fixed. See REG-61 for the tracked fix. |
| ~~**REG-60**~~ | Found during F5-V.2 live Aggregate Workbench re-verification, 2026-07-28 (`docs/POST_PUBLIC_PLAN.md` P4.3 / `docs/FRONTEND_STRATEGY_PLAN.md` F5-V.2); fixed 2026-07-28 (`docs/NEXT_EXECUTION_PLAN.md` P3.3) | LOW | **DONE (2026-07-28).** The Aggregate Workbench's post-commit "Saved." confirmation message was unobservable in practice: `commitDraft()`'s success handler set `msg.className="msg ok"` on the CURRENT render's message element, then immediately called `render()`, which rebuilds `#app` from scratch — including a fresh, blank `<span class="msg">` — wiping the confirmation before a user could ever see it. `invokeAction()`'s success handler had the identical shape (same file), so it was fixed too, not just `commitDraft()`. **Fix:** a module-level `pendingMsg` variable, set by the success handlers instead of mutating the doomed message element directly; `render()` now applies any pending message to the freshly-created `<span class="msg">` before clearing it. `NPDevGenerator/generator/src/main/resources/npdev-templates/workbench-page.html.mustache` (`commitDraft()`/`invokeAction()` around what was line 260, and the `render()` message-span creation). **Verified live** (not just unit-tested): WmsOffice regenerated + rebuilt, `ExpedicaoWorkbench.html`, real browser via ScrapForAI — logged in as `trial`/admin, opened a real `PreExpedicao` record, clicked Save, and both a DOM readback and a screenshot confirm the green "Saved." text is visible next to the Save button after the re-render (previously it would have been wiped). |
...
| ~~**REG-61**~~ | Split out of REG-59 during its live-recovery filing, 2026-07-28 (`docs/POST_PUBLIC_PLAN.md` P4.3 re-verification; see `docs/NEXT_EXECUTION_PLAN.md` P1.4); fixed 2026-07-28 (`docs/NEXT_EXECUTION_PLAN.md` P3.5) | HIGH | **DONE (2026-07-28).** Both needs carried verbatim from REG-59's filing, both fixed. **(a)** `DestructiveRecreationPass.executeNarrowTypeDropAndRecreate` now looks up the model's declared required-ness for the narrowed column (via `DesiredSchemaFactory`) and re-applies `NOT NULL` directly when the table is currently empty — a zero-row table no longer needs the backfill dance at all. A non-empty table still adds the column nullable exactly as before (a `NOT NULL ADD COLUMN` with no default fails outright against any existing row), leaving (b)'s refusal as the correct next line of defense. New `DestructiveRecreationPassRequiredColumnPreservationTest` (3/3): empty+required recreates genuinely `NOT NULL` (verified by both `IS_NULLABLE` and a rejected `NULL` insert, not just a cosmetic flag), non-empty+required stays nullable, non-required stays nullable regardless of row count. **(b)** `BackfillPass` now detects the exact shape the plan named — required + `UNIQUE`-constrained (single- or compound-field) + more than one row that would receive the same literal — and refuses by name (table.column, affected row count, a documented recovery recipe generalizing the out-of-band SQL WmsOffice used) instead of proceeding to a confusing duplicate-key failure once `UniqueConstraintPass` re-adds the constraint later in the same boot. Collected as its own refusal list, not folded into the pre-existing "no default at all" refusal — the two are different reasons. Did **not** invent a per-row-unique default expression language, per the plan's own scope decision. New `BackfillPassUniqueColumnRefusalTest` (2/2), RED-first per the register's own scenario: a required unique field + an existing 2-row table + a literal default refuses by name naming "2 existing row" and "per-row-unique" (and, Pass 1 staying read-only, never adds the column); a single affected row (cannot collide with itself) still backfills normally. `docs/SCHEMA_EVOLUTION.md#new-required-fields` documents the new refusal case and recipe. Full `com.finalexec.db` suite: 273/273, no regression. |
| **REG-62** | Investigated while closing F5-R1, 2026-07-28 (`docs/FRONTEND_STRATEGY_PLAN.md` F5-R1 / `docs/NEXT_EXECUTION_PLAN.md` P4.5) | LOW | **OPEN.** `allowedActions` (per-lifecycle-state action-rail gating, AW-P5) is authored today as a comma-separated string inside a lifecycle state's generic `metadata` map (`AutoPanelExpander.java:310`, reading `state.getMetadata().get("allowedActions")` via `splitCsv`) — not a typed schema field, and nothing validates an entry resolves to a real declared action. A typo (`GerarDemenda` instead of `gerarDemanda`) silently yields a missing button in production, the same class as REG-52/REG-53: the model is "valid," the app is wrong. **Why not simply typed:** the plan's original fix (F5-R1) assumed typing `allowedActions` as a JSON-Schema `string[]` plus validating each entry against the AutoPanel's declared actions would close this. Investigation found the second half is blocked on a real prerequisite gap — `AutoPanelSurfaceAst` has no `actions` field at all; an AutoPanel section's action list (e.g. `"actions": ["gerarDemanda"]` inside `transaction.sections[]`) lives inside that surface's own untyped `Map<String, Object> metadata`, the identical escape hatch `allowedActions` itself uses. JSON Schema alone cannot validate `allowedActions[i]` against a per-model, dynamically-declared set of action names, so typing the array without also resolving the action side first would look done without fixing the actual failure mode. **Fix, when picked up:** give AutoPanel section actions a typed AST home first (a real `List<String>` or richer type replacing the metadata-map convention), then add the typed `allowedActions` array (all 4 `model.schema.json` mirrors) plus a cross-reference check — likely in `PanelValidation.validateAutoPanels` (which already has both the concept's lifecycle and the AutoPanel's surfaces in scope, unlike `LifecycleValidation` which only sees the concept side) — that every `allowedActions` entry resolves to a declared action name, RED-first. **Not urgent:** corpus-checked 2026-07-27, 0 of 27 models use `allowedActions` at all, so no codemod/`BREAKING.md` entry is needed regardless of when this lands. |
| **REG-63** | Found while confirming panel-provenance manifests for R-G2 (`docs/REMEDIATION_PLAN.md`), 2026-07-29 | MEDIUM | **OPEN.** `AppGen/apps/_official/AuxScreen/definition/model.json` and `.../Pigmentampa/definition/model.json` both declare flows in a pre-DSL-stabilization shape — a top-level `input`/`out` on each step, a `type: "validate"` step kind — that the CURRENT `model.schema.json` rejects outright: `$.flows[N].steps[0].type` is not in the canonical enum (`invariantCheck`, `capabilityCall`, `generatedAction`, `emitEvent`, `scheduleEvent`, `return`, `branch`, `awaitEvent`, `createConcept`, `updateConcept`, `map`, `forEach`), and `input`/`out`/`name`/`steps` are rejected as undeclared additional properties at the flow level. Confirmed live: `Build-NpdevApp.ps1 -GenerateOnly` against both apps fails identically at `JsonModelParser.parse()` — schema validation runs BEFORE an AST is built, so neither app can be regenerated, and no compiled model/invocations catalog/live bundle can be produced for either at all. **Consequence for R-G2:** blocked both apps' single hand-written screen (`aux-screen.html`, `pigmentampa-editor.html`) from getting a panel-provenance manifest — there is nothing to bootstrap or hand-verify against without a compiled model. Both existing BUILT/running instances are unaffected (this only blocks regeneration with the current toolchain, not the already-running apps). **Fix, when picked up:** a real `npdev migrate` codemod for this flow-step shape (BREAKING.md's own standing rule: every DSL break ships its codemod in the same commit) — likely a mechanical `type: "validate"` → `invariantCheck` rewrite plus flattening the top-level `input`/`out`/`name` properties into whatever the current schema expects per step kind; needs a careful read of how many other AppGen apps (if any beyond these two) still carry this shape before scoping the codemod's coverage. Out of scope for a manifest-coverage pass — filed here so it is not lost. |
