...
| **REG-25** | LOW | Tenant match is case-sensitive → isolation-bucket fragmentation (NOT a cross-tenant bypass) | **DEFERRED (2026-07-21) with rationale.** Canonicalising casing requires migrating existing `tenant_id` values across the tenant registry AND every business table's `tenant_id` column -- a real data migration, disproportionate to a latent LOW that only bites a misconfigured same-tenant-different-casing setup. Track with the schema-evolution work, not as an inline hardening tweak. **Nuance (2026-07-24):** several runtime paths ALREADY lowercase the tenant (`DefaultConceptGateway`, `KernelRunner.normalizeTenantOrDefault`, `LoginThrottle`, `IdentityProvisioning`, `FileUploadController`) — but **stored** `tenant_id` values (registry + business tables) are not canonicalized, so the real shape is an *inconsistency* between write sites, not uniform case-sensitivity. Any full fix must reconcile all sites AND migrate stored data. |
