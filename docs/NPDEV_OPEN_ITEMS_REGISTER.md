...
| **REG-61** | Split out of REG-59 during its live-recovery filing, 2026-07-28 (`docs/POST_PUBLIC_PLAN.md` P4.3 re-verification; see `docs/NEXT_EXECUTION_PLAN.md` P1.4) | HIGH | **OPEN — the platform-level gap REG-59's WmsOffice recovery exposed but did not fix.** Two needs, carried verbatim from REG-59's filing: **(a)** `DestructiveRecreationPass.executeNarrowTypeDropAndRecreate` should preserve the original `NOT NULL`-ness of a narrowed column, so a table with zero existing rows never needs the backfill dance at all. **(b)** `BackfillPass`'s literal-default mechanism has no way to express a per-row-unique default for a `UNIQUE`-constrained, required column with existing rows — a flat literal backfill only trades "boot refuses, no default" for "boot refuses, duplicate key" once `UniqueConstraintPass` re-adds the constraint. Confirmed live on WmsOffice: `identity_roles.name` (5 rows), `identity_users.username` (6 rows), both tenant-scoped-unique. **Scope for (b), per the plan's own precedent:** do not invent a per-row-unique default expression language; instead make `BackfillPass` detect this exact shape (required + `UNIQUE`-constrained + >1 affected row + only a literal default available) and refuse with a specific, named diagnostic plus a documented recovery recipe — generalizing the out-of-band SQL WmsOffice needed — matching the existing bond-column refusal precedent rather than a second, differently-worded boot failure after (a) is fixed. **RED-first for (b):** a model with a required unique field and an existing 2-row table with a literal default; current behavior is a confusing duplicate-key failure after `UniqueConstraintPass`, post-fix behavior is a named refusal naming the affected rows and the recipe. Not yet started. |
