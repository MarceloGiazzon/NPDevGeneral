...
| **REG-39** | — | HIGH | **OPEN (filed 2026-07-24).** The identity pack (`NPDevContract/packs/identity/pack.json`) declares a `tokenVersion` field on `User`, but the generated `identity_users` schema (`SchemaRealizationEmitter` — both the `V1` CREATE TABLE and the `R__` additive column set) **omits `token_version` entirely** (it emits the other 8 columns). Meanwhile `LoginController` (and `JwtSigner`/`PasswordResetController`/`ControlPanelTenantUsersController`) unconditionally `SELECT … token_version FROM identity_users`; on any DB whose `identity_users` lacks the column that query throws and is swallowed into a generic `invalid_credentials`, so **every login fails**. Confirmed live: the WmsOffice DB has no `token_version` column and no login succeeds. NOTE: the **kernel-side** REG-23 read (`IdentityRoleLookup.tokenVersion`) is resilient (catches `SQLException`→returns 0), so REG-23 itself fails safe; the gap is that the column is never emitted into the schema at all. Likely a stale/incomplete propagation of the pack field into the schema-realization internal-table definition. Fix: emit `token_version` in both the CREATE and the additive column set for `identity_users` (and confirm on a FRESH generated app whether login is broken universally or only on upgraded DBs). Blocks the ARCH-upload P6 live login round-trip. |
