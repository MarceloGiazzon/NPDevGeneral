...
| **REG-55** | T2.B.4 live rehearsal, 2026-07-27 | MEDIUM | **FILED, not fixed — shallow finding, not deeply investigated.** During the 2.B.4 live rehearsal (additive-column change against `AppGen/apps/simple-product-h2local`, an H2Local-engine app), a REST write (`POST /api/flows/CreateProduct/execute` and the generated `POST /api/products`) threw `IllegalStateException: Ambiguous sandboxed plugin operation save ... PostgresPersistenceCapabilityAdapter` — a Postgres-specific adapter being considered (ambiguously) for capability dispatch on an app whose engine is H2Local, not Postgres. Not investigated beyond reproducing it once; unrelated to the file split itself (the read path and the schema-migration boot both worked correctly). Likely a capability-registry/plugin-resolution scoping gap (candidate adapters not filtered by the app's configured engine before ambiguity is checked) rather than anything in `com.finalexec.db`. Needs a proper RED-first repro before any fix. |
