...
`docs/REG16_CODEGEN_OUTPUT_ADVERSARIAL_REVIEW.md`, steered by
...
Findings document: `docs/REG16_FLOW_ORCHESTRATION_ADVERSARIAL_REVIEW.md`. **Headline: no CRITICAL, no
...
| REG-47 | sweep-closure | MED | **OPEN (filed 2026-07-25 by the post-programme sweep closure, `docs/POST_PROGRAMME_AUDIT_PLAN.md` §2.1).** **REG-36's failure mode on a different key.** A caller-supplied `correlationId` — it arrives on `EventPublishRequest` and the flow-execution endpoints as ordinary request data — is passed through `KernelRunner.normalizeCorrelationId`, which only `trim()`s it, and then written into **`TEXT` columns that are btree index key material in four tables**: `npdev_correlation_owner.correlation_id` (its **PRIMARY KEY**), plus indexed `correlation_id` in `npdev_event_store` (3 indexes), `npdev_flow_instance` (2) and `npdev_trace` (2) — 8 indexes in all. On Postgres an oversized *incompressible* value exceeds the btree index-entry limit and the write throws, exactly as REG-36 did: the event has already been published or the flow already executed, and the caller is told it failed. For `npdev_correlation_owner` the failing write is the one that establishes correlation ownership. MED for the same reasons as REG-36 — it needs an oversized incompressible value, and the compressible case inserts fine (see REG-36's correction). **Found by refusing to blanket-clear the `unbounded-caller-input` group during the sweep closure**; the 29 hits across those four stores are allowlisted under rule I2 pointing here, so they are tracked once rather than re-reported every run. **Recommended fix — reject, do not digest.** REG-36 digests because a model author's `idempotencyKeyField` may legitimately reference a large payload-derived value; a correlation id is caller-chosen metadata with no legitimate 10,000-character form, so a length ceiling enforced at `normalizeCorrelationId` (400 → HTTP 400, before any side effect) preserves the caller's exact value whenever it is sane and gives a clear error when it is not. Digesting would silently change an id the caller later uses for lookups on several surfaces (`@PathVariable correlationId` on the correlation and event-query controllers), so every one of them would have to agree — which is the argument for rejecting at the boundary instead. Regression test: an oversized correlationId is refused with 400 before any event is stored, and a normal one is stored byte-identical. |
| REG-45 | R4-F1 | MED | **OPEN (filed 2026-07-25 by REG-16-resid Round 4).** `docs/REG16_FLOW_ORCHESTRATION_ADVERSARIAL_REVIEW.md` §2. `DefaultExecutionAuthorizationPolicy.canResumeExecution` checks `RESUME_EXECUTIONS` permission + `sameTenant(requester, instance)`, but **nothing about the actor**. Any holder of that permission may resume *any* waiting instance in their tenant and receive the resulting `ExecutionResult`, which carries the flow's accumulated state — records read under the **original** actor's row-level `access.read` scope. The row-scoping LNCH-13 enforces on the concept surface has no equivalent on the execution surface. MED because `RESUME_EXECUTIONS` is operator-grade rather than something an ordinary user holds, and the target `executionId` is a UUID: a missing defence-in-depth layer behind a permission gate, not an open door. **Filed rather than fixed because the fix is a policy decision the platform has not made** — whether resume should require being the originating actor, holding an explicit override permission, or neither; inventing that answer inside a review round would be the wrong way to decide it. Regression test once decided: actor B with `RESUME_EXECUTIONS` in the same tenant must not receive actor A's flow state. |
| REG-46 | R5-F1 | MED | **OPEN (filed 2026-07-25 by REG-16-resid Round 5).** `docs/REG16_POSTGRES_ADAPTER_SQL_ADVERSARIAL_REVIEW.md` §3. `PersistenceCapabilityContract` has **no tenant parameter at all** (`findById(concept, id)`, `query(concept, criteria)`, `exists/unique(concept, field, value)`, …), so `PostgresPersistenceCapabilityAdapter` emits `select * from <table> where <id_col> = ?` with no tenant predicate — and *cannot* scope by tenant even in principle. The in-memory adapter is identical, so this is a gap in the port, not a backend asymmetry. Result: **two persistence routes with different isolation guarantees** — generated CRUD goes through `ConceptGateway` (tenant- AND row-scoped), while a flow's `persistence` capability step is unscoped. `NpdevPluginConfig` wires this adapter for every app whose `npdev.storage.mode` is not `in-memory`, and a step such as `persistence.findById(concept: Order, id: $input.orderId)` — an ordinary thing for a model author to write — returns any tenant's order. MED: reaching it needs a model that routes caller input into such a step plus a known target id (a UUID), and flow execution is itself gated by `canExecuteFlow`; it is the absence of a control on an alternate path rather than the bypass of one. **Filed rather than fixed because the fix is a breaking change to a published port** — threading `ExecutionContext` (or at least a tenant) through the contract and both adapters. Regression test once done: a flow step must not read a row outside the executing context's tenant. |
| REG-44 | R3-F3 | MED | **OPEN (filed 2026-07-25 by REG-16-resid Round 3).** `docs/REG16_CODEGEN_OUTPUT_ADVERSARIAL_REVIEW.md` §3. A model can declare `access.read`/`access.write` **and** set `crud.kernelControlled: false`, and it compiles clean — but `enforceWithConceptGateway` is emitted inside `{{#kernelControlled}}`, so the declared **write** rule is then never enforced anywhere. The read rule *is* still enforced (generated reads go through `conceptGateway` unconditionally), and that asymmetry is what makes it hard to notice: spot-checking read scope suggests the rules are live. Coarse `concept.*` permission checks and mutation audit are lost in the same move. `SemanticValidator` validates that the access expressions parse and are boolean-shaped, but never references the settings layer at all. Not privilege escalation — a model author disabling the setting is inside the trust boundary — but a **declared security rule silently not enforced, with no diagnostic**. Fix: reject, or warn on, the combination at compile time (`SemanticValidator` or `GeneratorFacade`, which already resolves `CRUD_KERNEL_CONTROLLED`). Left as a filed MEDIUM rather than fixed in-round because error-vs-warning is a product decision. Regression test: a model with `access.write` + `crud.kernelControlled: false` must not generate silently. |
