...
### REG-48, REG-49, REG-50 — findings filed by the first real ADR-0009 external-AI missions (M1–M3), 2026-07-27

Per `docs/EXTERNAL_AI_DELEGATION_PLAN.md` P5 and its own rule ("every finding gets a RED-first local
reproduction before any fix"): these three are **code-read-verified by re-reading the actual source
against each vendor's claim** (a stronger bar than "the vendor said so," matching the rigor of every
prior calibration/finding this session), but **no RED-first failing test has been written yet** for
any of them — they are filed OPEN, not DONE, pending that reproduction and an owner fix decision.
None of the three vendor findings behind these were hallucinated; all three point at real code paths.

| New item | From | Sev | Status |
|---|---|---|---|
| **REG-48** | M2-SEC-ROWAUTHZ, gemini F1 | HIGH | **OPEN.** `DefaultConceptGateway.delete()` (line ~268) still runs `evaluateRuleProfiles(requestContext, ruleProfilesForWriteBeforeCommit(...))` — semantic validation that reads the previous record's data via `requestContext`'s `previous` — **before** `enforcePermission(...)` at line ~273. This is the identical bug class REG-41 fixed in `save()` (authorization must run before any semantic validation that touches the previous record), but the fix was never applied to `delete()`. A caller with zero `concept.delete` permission or zero `access.write` row-scope can still trigger `evaluateRuleProfiles` first and potentially learn the row's state from whatever error a rule profile throws, before authorization ever runs. Fix sketch: mirror REG-41's `save()` reorder — move `enforcePermission`/`enforceRowWritable` before the `evaluateRuleProfiles` call in `delete()`, keeping the existing `previous` fetch (still needed for the row-scope check itself). |
| **REG-49** | M1-SEC-GENCODE, gemini F1 | HIGH | **OPEN.** For a concept with a custom create/update flow, the generated CRUD service swaps the row-level-policy-enforcing `enforceWithConceptGateway(...)` (which calls `ConceptGateway.save()`) for the custom flow's own steps (`enforceWithCreateFlow`/`enforceWithUpdateFlow`, which run the declared Flow via `kernelRunner.execute(...)`), and nothing re-establishes the row-level `access.write` check the flow bypassed. Verified concretely on the real generated code for `wmsoffice`: `CrossDockingServiceBase.java` never calls `enforceWithConceptGateway` on **either** create or update (both routed through `AtivarCrossDocking`/`CancelarCrossDocking`); `MovimentoServiceBase.java` calls it correctly on **create** but not on **update** (routed through `ConfirmarMovimentacao` instead) — the vendor's claim was precise down to which operation is affected per concept. The persist step itself (`saveWithIntegrityMapping` → `persistence.save` → `conceptStore.save` directly) also never goes through `conceptGateway`. This is a generator-level gap (`business-ui-app.mustache`/`service-base.mustache`'s flow-integration branch), not app-specific — any concept anywhere with a custom mutation flow has the same hole. Fix sketch: the flow-integration branch needs its own row-level `access.write` check (via `ConceptGatewaySemanticPolicy`, not skipped when a custom flow substitutes for the default save/delete call). |
| **REG-50** | M3-SEC-TENANT, gemini F1+F2 (same root cause) | HIGH | **OPEN.** `PostgresPersistenceCapabilityAdapter`'s `TableColumns.unavailable()` (returned by `loadTableColumns` on a genuine `SQLException`, line ~562) is indistinguishable from "this table legitimately has no `tenant_id` column" — both make `hasColumn(TENANT_COLUMN)` return `false`, so a transient metadata-read failure on a tenant-scoped table silently falls back to the unscoped `findById`/`delete`/`exists` overloads (verified at lines ~420, ~455, ~481) instead of failing closed. **Compounding SQL-injection surface on the same fallback:** `resolveCriteriaColumn`/`normalizeCriteria`, when `!tableColumns.isAvailable()`, use `toDbColumn(runtimeField)` directly as a SQL column identifier with zero sanitization (verified: `toDbColumn` only case-converts, never strips/escapes/whitelists characters) — a caller-influenced field name reaching this fallback path is concatenated into the SQL string unchecked. Minor related note (M3 gemini F3, MEDIUM, same section): `TenantIsolationPolicy.STRICT_EQUALS.normalize()` only trims (case-sensitive), while `ExecutionContext.normalizeTenantId()` lowercases (per its own REG-25 comment) — a real inconsistency if `STRICT_EQUALS` is ever compared against a tenantId that bypassed `ExecutionContext`'s constructor, though the direction is fail-closed (spurious mismatch) rather than fail-open. Fix sketch: `loadTableColumns` needs to distinguish "queried, zero columns" from "query threw" (e.g. a tri-state or a thrown/logged error on genuine `SQLException` instead of silently swallowing it into the same `unavailable()` used for "column doesn't exist"), and the unavailable-metadata fallback in `resolveCriteriaColumn`/`normalizeCriteria` needs a character-class whitelist before any field name reaches SQL text. |
