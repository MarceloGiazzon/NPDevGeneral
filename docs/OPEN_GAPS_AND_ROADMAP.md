...
| GATE-REL-1 | **The workspace slimness policy and `run-beta-release-gate.ps1` are in direct, structural tension — one requires the other's precondition to be ABSENT.** The pre-commit slimness hook refuses any commit while `scripts\quality\json-schema-validator\node_modules` exists (listed as a "rebuildable directory [that] must not remain in the workspace"; its 542 files also push the workspace over the 3000-file limit on their own). But the beta-release gate is a release-evidence **aggregator**: its first required report (of 36, each needing to be less than `maxReportAgeHours=24` old) is `json-schema-validator-tests`, which needs `node_modules` **present** to run at all. Found 2026-07-21 (platform-column round, T7.2) while actually attempting a full run: restoring it is a one-liner (`npm ci` in `scripts\quality\json-schema-validator`; `package.json`/`package-lock.json` are tracked, npm is on PATH) and confirmed to bring back exactly 542 files, but **you cannot hold it present and commit at the same time** — assembling the full beta-release evidence set requires install → run all 36 gates → the workspace is now over-limit and un-committable until `node_modules` is removed again, which invalidates the very report just produced. Two candidate resolutions, needing an owner's call, not a unilateral pick: **(a)** exempt this one vendored, tracked-via-lockfile directory from the slimness policy (it is reproducible from `package-lock.json`, the same argument that lets other lockfile-backed dirs stay gitignored-but-tolerated elsewhere in this repo), or **(b)** teach `run-beta-release-gate.ps1` (or the `json-schema-validator-tests` check specifically) to provision `node_modules` on demand as part of running the gate and tear it down again before finishing, so it is never present at the moment a commit is attempted. Neither is implemented; this row exists so the tension is a tracked decision, not something the next person re-discovers via a failed commit | Quality-gate / workspace-policy conflict | OPEN — needs an owner decision between (a) exempt the directory or (b) gate provisions it on demand; not attempted as a workaround | P3 | S |
