...
| GATE-OBS-1 | RuntimeHost gate's **sole** remaining red check is `runtime-surface-reports-current`, driven by 6 named sub-checks: classification (`service-buckets-are-exclusive` overlappingServices=18; `controller-namespaces-match-convergence-buckets` mismatches=12; `service-namespaces-match-convergence-buckets` mismatches=17) and footprint (`controller-namespace-convergence-is-clean` 12; `service-namespace-convergence-is-clean` 17; `supported-controller-footprint-stays-minority` supported=25 vs excluded=7). This is **runtime-surface namespace/bucket convergence drift**, not a test failure — `:test` reports BUILD SUCCESSFUL first. Until it is resolved, `run-runtimehost-gate.ps1` always exits 1, which trains everyone to ignore the gate | Quality-gate evidence drift | OPEN (attributed by name + counts 2026-07-21, closeout C7.1) | P3 | M |
| IT-EXTPG-1 | 10 `integrationTest` failures need an **externally-configured Postgres** and cannot pass in a default checkout: `JwtAuthExternalBetaIT` (8), `PublicationRollbackE2EIT` (1), `TenantIsolationE2EIT` (1). All are `ApplicationContext` load failures under `activeProfiles=["test","postgres","external-beta"]`, root cause `NoSuchBeanDefinitionException: No qualifying bean of type 'javax.sql.DataSource'`. **Precondition to run them:** a reachable Postgres wired into the `postgres` profile's datasource properties. Unlike `SchemaLifecycleExecutorPostgresProofMatrixTest` these are **not** Testcontainers-backed, which is why that twin passes (25/25) in the same run. Unrelated to schema lifecycle; unchanged across four rounds | Test-environment precondition | OPEN — tracked so it is not re-derived a fifth time | P4 | M |
